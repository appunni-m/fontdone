...
includes only the default oracle needed by the instrumented parity matrix. The
optional-feature contract is a separate gate already exercised by
`make test-parity-smoke`; set `COVERAGE_PREPARE_OPTIONAL_FEATURES=1` when an
isolated coverage invocation explicitly needs those non-instrumented bundles.
The ABI-only package preflight remains a separate `make coverage-abi-preflight`
gate already exercised by `make test-fast`. The
...
| 121 | Working tree (OT-SVG missing-document route classification) | Promoted the maintained `ftimage.FT_GLYPH_FORMAT_SVG.missing_svg_document_rejected` case from the generic-fallback bucket to explicit real parity in `scripts/check_public_api_inputs.py`. The case already compared the pinned C load result, empty SVG slot state, Rust FFI, C ABI, and WASM outputs for the existing SVG range-gap fixture, so no runtime behavior, fixture, or denominator changed. Full parity run `6e4d3eda-9104-4af2-b72c-77b54c862a3d` passed 7,567 / 7,567 runnable comparisons with 0 failures and 3 pending safety-extension cases; the route audit now has 7,570 concrete cases, 5,301 real-parity rows, 0 generic-fallback rows, and 0 pending routes, with 218 / 218 function evidence in each surface. C-ABI contract run `ccde80fe-407a-4d12-a4f7-a0fe5c5cee3b` reports 10 / 12 categories complete, C01.7 at 5,310 / 5,310, and C08.3 at 7,567 / 7,567; the remaining contract debt is the Windows import-library item and four fresh target-lane bundles. The measured coverage snapshot is unchanged at 50,088 / 54,423 lines, 9,986 / 12,602 branches, 3,411 / 3,846 functions, and 68,883 / 75,644 regions because this was a classifier/docs-only change. |
...
| 125 | Working tree (coverage process-sharding speed) | Added configurable `COVERAGE_UNIFIED_SHARDS` to the split all-lane runner. The instrumented parity binary now runs two disjoint case shards per Rust FFI, C ABI, and WASM backend (six processes total), writes process-local LLVM profiles, and merges all profiles without changing the 7,568-case matrix or coverage denominator; per-shard summaries now report truthful 3,784-case totals. Clean-commit Coverage MCP run `9a39257b-66fb-4dd2-ab29-46835cae54a3`, snapshot `04c522e6-da5a-43c2-8d98-19ccf176be8e`, completed in 63.799 seconds with a 45.60-second cold instrumented build; warm repeat `a7c92ef5-c657-4d77-8586-256e1505f770`, snapshot `c3af13b1-e969-4538-aeaf-8bbe171dd66e`, completed in 17.498 seconds. Both retained 50,122 / 54,388 lines, 9,989 / 12,599 branches, 3,410 / 3,822 functions, and 68,864 / 75,559 regions; every shard passed 3,784 / 3,784 comparisons. The optimization addresses measured process-local counter contention and leaves oracle outputs, parity inputs, pending safety exclusions, and C-contract status unchanged. |
| 126 | Working tree (adaptive coverage shard speed) | Measured the six-process/two-shard baseline at 19.747 seconds on a warm instrumented binary, then made the default adaptive: three shards per backend on hosts with at least 12 logical CPUs, two on smaller runners. Three current three-shard Coverage MCP runs `f422f061-f58a-4084-8339-347bf31ba296` (snapshot `1ccbe0d4-b870-4abf-b09e-b97ae187a06e`), `076f95b7-1b71-4e70-9b3c-d72e27b25a6d`, and final `3a435c00-9b3e-4abe-9070-89a4f3566e7f` (snapshot `ecc4a9dd-efe6-4db7-9c82-32cdf7f1bdf3`) completed in 16.411, 16.377, and 15.414 seconds; all nine processes passed 2,524 / 2,525 disjoint comparisons and retained 50,125 / 54,388 lines, 9,995 / 12,599 branches, 3,410 / 3,822 functions, and 68,870 / 75,559 regions. A four-shard trial passed but regressed to 26.974 seconds under sustained load, so it was rejected. The remaining cold delay is the 45-second instrumented rebuild, not MCP ingestion or oracle comparison. |
