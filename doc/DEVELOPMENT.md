...
increase coverage. Coverage MCP run
`45f11d77-3e68-4285-9e94-b4c5655bcb96` used the argument-based selector against
baseline snapshot `a73abe7c-a4d0-4d76-8a8d-1e15a247cfd3` and ingested selected
snapshot `3989c118-c980-4d7b-a2ce-1acbf7f2b97e`. Because the managed artifact
carried stale project source metadata, the same local report was explicitly
imported with commit provenance as snapshot
`ec338637-4448-4678-bf75-a163f43ba982` at commit `b8809ac`. The source-matched
review shows `src/font.rs:1287` covered, where the baseline had one uncovered
branch. This selected report is marked `measurement_scope.kind=selected_subset`
and `complete=false`, so it is reachability evidence rather than a replacement
for the full-denominator score; test attribution is unavailable.
...
### Batch 326: public PCF reader field-truncation boundaries

This batch adds exactly 50 public `FT_New_Memory_Face` inputs under the
manifest case `error_pcf_reader_truncated_fields`. Their exact runtime IDs are
`freetype.FT_New_Memory_Face.error_pcf_reader_truncated_fields@batch326-pcf-reader-001`
through
`freetype.FT_New_Memory_Face.error_pcf_reader_truncated_fields@batch326-pcf-reader-050`.
The maintained PCF assets are generated by
`scripts/font_generation/build_pcf_fixtures.py:180-260` and tracked under
`tests/fixtures/input/fonts/pcf/`:

- IDs 001-008 truncate `PCF_PROPERTIES` at payload lengths 4, 5, 6, and 7
  bytes, with both byte orders and repeated boundaries.
- IDs 009-016 truncate compressed metrics at lengths 4 through 9 bytes.
- IDs 017-024 truncate uncompressed metrics at lengths 4 through 11 bytes.
- IDs 025-032 truncate the accelerator header at lengths 4, 8, and 12
  through 17 bytes.
- IDs 033-040 truncate the bitmap header at lengths 4 through 7 bytes.
- IDs 041-050 truncate the encoding header at lengths 4 through 9 bytes,
  repeating the shortest boundaries with both byte orders.

Every variant has its own `batch326-pcf-reader-###` ID, asset, reason, and
coverage label in
`tests/fixtures/inputs/public-api/freetype.FT_New_Memory_Face.json`. The
reason identifies the exact table reader and payload length. The generator
places the selected table last in the TOC so the pinned C reader reaches a
real end-of-stream while all preceding required tables remain complete; this
avoids claiming a C reader boundary when only the Rust table slice is short.

The source review used the pinned FreeType 2.14.3 PCF implementation at
`freetype/src/pcf/pcfread.c:93-223` for TOC bounds,
`:478-674` for properties, `:677-809` for metrics, `:812-930` for bitmaps,
`:975-1136` for encodings, and `:1183-1245` for accelerators. A successful
TOC parse followed by a bounded table-reader failure enters
`PCF_Face_Init` at `freetype/src/pcf/pcfdrivr.c:241-315`, where the raw input
is retried through the optional compressed-stream drivers and the final public
`FT_New_Memory_Face` result is `FT_Err_Unknown_File_Format` (numeric code 2).
The PCF probe/TOC failures remain the separate
`FT_Err_Invalid_Stream_Operation` class (numeric code 85); this batch targets
the later reader failures.

The first focused parity run found the implementation mismatch on the eight
properties variants: C returned code 2 while Rust returned code 85 from
`src/font.rs:646-690`. The Rust properties parser had classified every
short scalar/table read as `pcf_stream_operation`, unlike the metrics,
bitmap, encoding, and accelerator readers, which already used the
`pcf_unknown_file_format` path. The fix keeps TOC errors unchanged and maps
the post-TOC properties-reader failures to `UnknownFileFormat`, matching the
pinned driver retry behavior. No unit test was added or used for coverage.

After moving each target table to EOF, focused public parity passed all 50
variants (`50/50`) across Rust FFI, C ABI, and WASM. The API audit classified
the 50 concrete inputs as real parity cases, and all 50 pinned C results were
the expected error with no face created.

The first complete run exposed that the initial properties mapping was too
broad: the ten existing `c121-pcf-invalid-*` TOCs keep
`FT_Err_Invalid_Stream_Operation` (85) because their properties entry is
followed by zero-length entries, so it is not the final sorted TOC entry. The
Rust parser now receives that final-entry fact from `pcf_tables`; properties
reader failures use `UnknownFileFormat` only for the final TOC table that
FreeType adjusts to the actual end of the stream, while earlier entries retain
the stream-operation classification. Focused parity then passed all ten C121
probes and all 161 PCF-filtered concrete cases. The authoritative full
Coverage MCP run `bc353e7b-129b-491c-b36e-0e45e7353964` passed and ingested
snapshot `6e397a43-632a-42ef-9105-d9da5f445b29`: 90,979/94,489 regions
(96.28528188466838%), 66,008/68,003 lines (97.06630589826924%),
