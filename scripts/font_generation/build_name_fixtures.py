...
VARIABLE_OUT_DIR = ROOT / "tests" / "fixtures" / "input" / "fonts" / "variable"
...
def write_format1_langtag() -> None:
    ...
    write_name_payload(
        BASE_STATIC,
        ROOT / "tests" / "fixtures" / "input" / "fonts" / "sfnt" / "name-format1-langtag.ttf",
...
def write_format1_langtag_malformed_controls() -> None:
    ...
    write_name_payload(
        BASE_STATIC,
        ROOT / "tests" / "fixtures" / "input" / "fonts" / "sfnt" / "name-format1-langtag-count-missing.ttf",
        count_missing,
    )
    ...
    write_name_payload(
        BASE_STATIC,
        ROOT / "tests" / "fixtures" / "input" / "fonts" / "sfnt" / "name-format1-langtag-record-overflow.ttf",
        record_overflow,
    )
    ...
    write_name_payload(
        BASE_STATIC,
        ROOT / "tests" / "fixtures" / "input" / "fonts" / "sfnt" / "name-format1-langtag-string-oob.ttf",
        string_oob,
    )
...
def write_bad_storage_name_table() -> None:
    # The public SFNT face loader treats `name` as optional, but its pinned
    # internal load_name service returns Name_Table_Missing for this exact
    # record-array overflow.  The contract harness invokes that service
    # through a controlled oracle diagnostic.
    payload = (
        (0).to_bytes(2, "big")
        + (1).to_bytes(2, "big")
        + (6).to_bytes(2, "big")
