...
OUT_DIR = ROOT / "tests" / "fixtures" / "input" / "fonts" / "cmap"
CHARMAP_OUT_DIR = ROOT / "tests" / "fixtures" / "input" / "fonts" / "charmap"
...
def build_codepoint_zero_mapped_font() -> None:
    ...
    CHARMAP_OUT_DIR.mkdir(parents=True, exist_ok=True)
    out = CHARMAP_OUT_DIR / "format12-codepoint-zero-mapped.ttf"
    if out.exists() or out.is_symlink():
        out.unlink()
    font.save(out, reorderTables=True)
