...
The same generator emits the `charset-registry-iso8859.bdf` and
`charset-registry-iso646.bdf` controls. Their `CHARSET_REGISTRY` and
`CHARSET_ENCODING` pairs exercise the pinned Unicode-charmap classifications
for `ISO8859/1` and `ISO646.1991/IRV`; they are consumed only by maintained
input-only parity variants.
