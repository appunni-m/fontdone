...
| `build_cff_fixtures.py` | Synthetic and derived | Project-authored CFF1/CFF2, TrueType control, valid cubic-bbox extrema probes, source-reviewed global-subroutine EOF and fixed-operand arithmetic controls, random-operator Private-dictionary boundary controls including one-byte/reserved-number/reserved-operator parser cases, and malformed CFF1/CFF2 table and INDEX data, plus malformed CID CFF derivatives of the maintained OFL-1.1 `FDArrayTest257` face. |
