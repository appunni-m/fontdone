...
| `build_render_fixtures.py` | Synthetic | Project-authored outlines and TrueType programs, including the malformed 40-component composite point-limit control used by Batch274. |
...
| `generate_malformed_bdf_fixtures.py` | Synthetic | Project-authored BDF text, including valid atom/integer/cardinal property controls, malformed constructor variants, the Batch232 numeric-prefix/no-value/sign/saturation property matrix, the Batch235 malformed `SIZE` decimal-prefix fixed-strike matrix, and the Batch259 zero-height, unsigned-short-saturation, and normalized 2/4/8-bit bitmap-limit controls. |
...
| `build_sbix_fixtures.py` | Synthetic derivative | Repository-generated `hinter-control-matrix.ttf`; adds project-authored 24 ppem `sbix` controls for PNG/JPEG/TIFF/RGBL/unknown graphic types, `dupe`/`flip` recursion, missing and malformed glyph ranges, and malformed optional-table face-open guards, including out-of-range strike offsets and truncated ppem/ppi fields. The pinned oracle build has PNG decoding disabled, so the public error paths remain deterministic. |
