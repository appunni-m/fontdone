#!/usr/bin/env python3
"""Generate the wasm32 ABI inventory and JavaScript host declarations."""
...
def render_npm_typescript() -> str:
    return """// Generated by scripts/generate_wasm_contract.py. Do not edit.

/** Inputs accepted by the browser and Node.js Wasm initializer. */
...
/**
 * Instantiates a fresh engine. In browsers, no source fetches `fontdone.wasm`
 * relative to the package entry point. Node.js uses its conditional entrypoint
 * to read the same bundled asset from disk.
 */
...
def contract(text: str) -> dict[str, object]:
    ...
    return {
        ...
        "endianness": "little",
        "supported_hosts": [
            "browsers with ESM, fetch, WebAssembly, and JavaScript BigInt integration",
            "Node.js >= 20",
        ],
        "integer_lowering": {
...
def main() -> None:
    ...
    npm_ts_content = render_npm_typescript()
    expected = (
        (JSON_OUTPUT, json_content),
        (TS_OUTPUT, ts_content),
        (NPM_TS_OUTPUT, npm_ts_content),
    )
    stale = [
        path
