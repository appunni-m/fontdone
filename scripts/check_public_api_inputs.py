...
FTERRDEF_EXACT_ERROR_BATCH = {
    ...
    (
        "load_glyph",
        "fterrdef.FT_Err_Code_Overflow.unterminated_control_flow_returns_error",
    ),
    ("load_glyph", "fterrdef.FT_Err_Code_Overflow.push_instruction_truncation_returns_error"),
    (
        "new_memory_face",
        "fterrdef.FT_Err_Corrupted_Font_Header.zero_units_per_em_rejected_before_autohint",
    ),
    (
        "fterrdef.truetype_context_allocation_failure",
    ...
    (
        "load_glyph",
        "fterrdef.FT_Err_Stack_Overflow.cff_stack_overflow_translates_to_invalid_file_format",
    ),
    (
        "load_glyph",
        "fterrdef.FT_Err_Stack_Underflow.cff_underflow_translates_to_invalid_file_format",
    ),
    ("freetype.attach_stream", "fterrdef.FT_Err_Syntax_Error.malformed_afm_pair_count"),
    ("load_glyph", "fterrdef.FT_Err_Too_Few_Arguments.tt_interpreter_argument_underflow"),
    (
        "load_glyph",
        "fterrdef.FT_Err_Too_Few_Arguments.cff_underflow_does_not_surface_too_few_arguments",
    ),
...
PINNED_C_SUCCESS_FOR_DECLARED_ERROR_CASES = set()
...
FTDRIVER_SUCCESS_OPERATIONS = {
    "ftdriver.glyph_to_script_map_effect",
...
C_ABI_SOURCE = ROOT / "fontdone-c-abi" / "src" / "implementation.rs"
C_ABI_HEADER = ROOT / "fontdone-c-abi" / "include" / "fontdone_ffi.h"
WASM_ABI_SOURCE = ROOT / "fontdone-wasm" / "src" / "implementation.rs"
...
REAL_PARITY_OPERATIONS = {
    "fterrdef.open_face_non_driver",
    "fterrdef.sfnt_load_name_diagnostic",
    "new_memory_face",
    "set_pixel_sizes",
    "set_char_size",
    ...
    "ftmodapi.module_class_lifecycle",
    "ftmodapi.add_get_remove_module",
    ...
    "ftpfr.get_pfr_advance",
    "ftpfr.get_pfr_kerning",
    "ftpfr.get_pfr_metrics",
...
EXPECTED_PUBLIC_FUNCTION_COUNT = 218
RUNTIME_FUNCTION_EVIDENCE_CATEGORIES = {
    "real-parity",
    "real-null-validation",
}
PARITY_FRONTENDS = ("rust", "c_abi", "wasm")
...
def check_file(subject: ManifestSubject) -> list[str]:
    ...
    for index, case in enumerate(cases):
        ...
        contract_scope = case.get("contract_scope", "pinned_c")
        if contract_scope not in {"pinned_c", "fontdone_safety_extension"}:
            errors.append(
                f"{subject.subject_id}: cases[{index}] has unknown contract_scope "
                f"{contract_scope!r}"
            )
        if contract_scope == "fontdone_safety_extension":
            classifiers = case.get("classifiers", [])
            ...
            if case.get("schema") != "abi_safety_result":
                errors.append(
                    f"{subject.subject_id}: cases[{index}] Fontdone safety extension "
                    "must use abi_safety_result"
                )
            if (
                not isinstance(classifiers, list)
                or "scope:fontdone-safety-extension" not in classifiers
            ):
                errors.append(
                    f"{subject.subject_id}: cases[{index}] Fontdone safety extension "
                    "must declare scope:fontdone-safety-extension"
                )
            if not isinstance(source_refs, list) or not source_refs:
                errors.append(
                    f"{subject.subject_id}: cases[{index}] Fontdone safety extension "
                    "must cite the pinned C source establishing its non-contract boundary"
                )
...
def check_ffi_abi_surface(audit_json: Path) -> list[str]:
    ...
    for export in sorted(c_exports):
        if export not in public_functions:
            errors.append(f"c-abi exports non-FreeType C symbol {export}")
    header_text = C_ABI_HEADER.read_text()
    header_symbols_text = re.sub(
        r'^\s*#\s*include\s+"[^"]+"\s*$',
        "",
        header_text,
        flags=re.MULTILINE,
    )
    helper_symbols = sorted(
        set(re.findall(r"\bfontdone_[A-Za-z0-9_]*\b", header_symbols_text))
    )
    if helper_symbols:
        errors.append(
            "c-abi public header exposes fontdone_* helper symbols: "
            + ", ".join(helper_symbols)
        )
    for export in sorted(c_exports):
        if not re.search(r"\b" + re.escape(export) + r"\s*\(", header_text):
            errors.append(f"c-abi header missing exported symbol {export}")
    ...
    for export in sorted(extra_wasm):
        errors.append(f"wasm exports unexpected symbol {export}")
    for export in sorted(missing_wasm):
        errors.append(f"wasm missing expected symbol {export}")
...
def exported_functions(path: Path) -> set[str]:
    ...
    pattern = (
        ...
        r"(?:(?:///[^\n]*\n)\s*)*"
        r"pub\s+extern\s+\"C\"\s+fn\s+([A-Za-z_][A-Za-z0-9_]*)"
    )
    return set(re.findall(pattern, text))


def effective_expect_error(
    operation: str,
    case_id: str,
    declared: bool,
    params: dict[str, object],
) -> bool:
    """Normalize legacy exact-error glyph-name declarations after expansion."""
    if declared or operation != "freetype.get_glyph_name":
        return declared
    expected_status = params.get("expected_status")
    if (
        isinstance(expected_status, str)
        and expected_status.startswith("FT_Err_")
        and expected_status != "FT_Err_Ok"
    ):
        return True
    return case_id in {
        "freetype.FT_Get_Glyph_Name.error_no_names_or_invalid_glyph",
        "freetype.FT_Get_Glyph_Name.error_null_face_or_buffer",
    }
        ...
        for case in data.get("cases", []):
            ...
            if isinstance(variants, list) and variants:
                for variant in variants:
                    ...
                    expect_error = effective_expect_error(
                        operation,
                        case_id,
                        bool(
                            variant.get(
                                "expect_error",
                                case.get("expect_error", False),
                            )
                        ),
                        params,
                    )
                    rows.append(
                        ConcreteInput(
                            ...
                            or compare.get("mode") == "exact_error"
                            or (
                                expect_error
                                and operation == "freetype.get_glyph_name"
                            )
                            ...
                            contract_scope=str(
                                variant.get(
                                    "contract_scope",
                                    case.get("contract_scope", "pinned_c"),
                                )
                            ),
                ...
                rows.append(
                    ConcreteInput(
                        ...
                        expectation_status=expectation_status,
                        contract_scope=str(case.get("contract_scope", "pinned_c")),
...
def ftcache_manager_new_pending_reason(row: ConcreteInput) -> str | None:
    ...
    if not row.operation.startswith("ftcache."):
        return None
    if (
        row.case_id == "ftcache.FTC_Manager.owns_faces_sizes_and_cache_nodes"
...
def ftcolor_palette_pending_reason(row: ConcreteInput) -> str | None:
    ...
    if row.case_id in disabled_cases and unresolved_assets_reason(row) is None:
        return None
    exact_cases = {
        "ftcolor.FT_Palette_Data_Get.error_color_layers_disabled": (
...
def ftgxval_subsystem_pending_reason(row: ConcreteInput) -> str | None:
    """GX/classic-kern rows are maintained by the focused validator route."""
...
def ftmodapi_subsystem_pending_reason(row: ConcreteInput) -> str | None:
    ...
    if row.case_id == "ftmodapi.FT_Module_Class.fields_drive_module_lifecycle":
        if row.operation == "ftmodapi.module_class_lifecycle":
            return None
        if exact_error_public_route(row.operation, row.case_id, row.expect_error):
            return None
        return (
    ...
    if row.case_id == "ftmodapi.FT_Remove_Module.removes_installed_module":
        if row.operation == "ftmodapi.add_get_remove_module":
            return None
        if exact_error_public_route(row.operation, row.case_id, row.expect_error):
            return None
        return (
...
def property_service_pending_reason(row: ConcreteInput) -> str | None:
    maintained_routes = {
        "ftdriver.FT_Prop_GlyphToScriptMap.map_mutation_affects_autohint_script": (
            "ftdriver.glyph_to_script_map_effect",
            "actual_glyph_to_script_map_effect",
        ),
        "ftdriver.FT_Prop_IncreaseXHeight.limit_changes_autohint_x_height": (
            "ftdriver.increase_x_height_effect",
            "actual_increase_x_height_effect",
        ),
    }
    maintained = maintained_routes.get(row.case_id)
    if (
        maintained is not None
        and row.operation == maintained[0]
        and row.params.get("runtime_route") == maintained[1]
        and unresolved_assets_reason(row) is None
    ):
        return None
...
def ftincrem_subsystem_pending_reason(row: ConcreteInput) -> str | None:
    ...
    if (
        ...
        == "actual_incremental_glyph_lifecycle"
        and unresolved_assets_reason(row) is None
    ):
        return None
    if (
    ...
    if (
        ...
        return None
    if (
        row.case_id == "ftincrem.FT_Incremental_MetricsRec.horizontal_override_applied"
        and row.operation == "ftincrem.incremental_metrics_override"
        and row.params.get("runtime_route") == "actual_incremental_metrics_override"
        and unresolved_assets_reason(row) is None
    ):
        return None
    if (
...
def ftincrem_real_parity_reason(row: ConcreteInput) -> str | None:
    ...
    if (
        ...
        return (
            "FT_PARAM_TAG_INCREMENTAL face open, glyph-data acquisition, "
    ...
    if (
        ...
        return (
            "FT_Incremental_GetGlyphMetricsFunc mutates the horizontal seeded "
...
def done_glyph_lifecycle_pending_reason(row: ConcreteInput) -> str | None:
    ...
    if (
        ...
        return None
    if row.case_id == "ftglyph.FT_Done_Glyph.success_releases_owned_glyph":
        glyph_kinds = row.params.get("glyph_kinds")
        creation_paths = row.params.get("creation_paths")
        if (
            isinstance(glyph_kinds, list)
            and {"outline", "bitmap", "svg_optional"} <= set(glyph_kinds)
            and isinstance(creation_paths, list)
            and {"FT_Get_Glyph", "FT_Glyph_To_Bitmap"} <= set(creation_paths)
            and row.params.get("destroy_order") == "glyph_before_library"
            and {"outline_font", "bitmap_strike_font"} <= row.assets.keys()
        ):
            return None
    if row.case_id == "ftglyph.FT_Done_Glyph.lifetime_before_library_done":
        destroy_orders = row.params.get("destroy_orders")
        if (
            isinstance(destroy_orders, list)
            and {"glyph_before_library", "library_before_glyph_facade"}
            <= set(destroy_orders)
            and "outline_font" in row.assets
        ):
            return None
...
def done_glyph_lifecycle_real_parity_reason(row: ConcreteInput) -> str | None:
    """Owned/null glyph routes with direct four-lane lifetime execution."""
    if row.operation != "ftglyph.done_glyph":
        return None
    supported_cases = {
        "ftglyph.FT_OutlineGlyphRec.owns_outline_arrays",
    ...
    if row.case_id not in supported_cases:
        return None
    if done_glyph_lifecycle_pending_reason(row):
        return None
    return (
        "direct pinned C, Rust FFI, C ABI, and WASM glyph destruction "
        "with exact owned-payload and lifetime output"
    )
...
def ftglyph_subsystem_pending_reason(row: ConcreteInput) -> str | None:
    ...
    if (
        ...
        and row.params.get("runtime_route") == "actual_custom_glyph_lifecycle"
    ):
        return None
...
def ftparams_subsystem_pending_reason(row: ConcreteInput) -> str | None:
    ...
    if (
        ...
        and row.params.get("runtime_route")
        == "actual_incremental_state_lifecycle"
        and unresolved_assets_reason(row) is None
    ):
        return None
...
def ftparams_incremental_real_parity_reason(row: ConcreteInput) -> str | None:
    """Stored incremental-interface state with byte and metric callbacks."""
    if (
        row.case_id
        == "ftparams.FT_PARAM_TAG_INCREMENTAL.incremental_interface_used_for_glyph_load"
        and row.operation == "ftincrem.incremental_state_lifecycle"
        ...
        return (
            "FT_Open_Face retains the FT_PARAM_TAG_INCREMENTAL interface "
...
def ftimage_subsystem_pending_reason(row: ConcreteInput) -> str | None:
    ...
    if row.case_id == "ftimage.FT_OUTLINE_OWNER.destruction_ownership_behavior":
        operations = row.params.get("operations")
        flags_matrix = row.params.get("flags_matrix")
        if (
            isinstance(operations, list)
            and {"FT_Outline_New", "FT_Outline_Done"} <= set(operations)
            and isinstance(flags_matrix, list)
            and {"FT_OUTLINE_NONE", "FT_OUTLINE_OWNER"} <= set(flags_matrix)
            and row.params.get("n_points") == 4
            and row.params.get("n_contours") == 1
        ):
            return None
    ...
    if (
        ...
        return None
    if (
        row.case_id == "ftimage.FT_Raster_Funcs.callback_slots_match_registered_renderers"
        and row.operation == "renderer.class_probe"
    ):
        return None
    if (
        row.case_id == "ftimage.FT_GLYPH_FORMAT_PLOTTER.source_emitter_inventory"
...
def ftimage_raster_class_real_parity_reason(row: ConcreteInput) -> str | None:
    if (
        row.case_id == "ftimage.FT_Raster_Funcs.callback_slots_match_registered_renderers"
        and row.operation == "renderer.class_probe"
    ):
        return (
            "FT_Raster_Funcs glyph-format and callback-slot presence compare through "
            "the maintained standard, grays, SDF, and bitmap-SDF renderer class "
            "probe across pinned C, Rust FFI, C ABI, and WASM ABI"
        )
    return None


def ftimage_glyph_format_emitter_real_parity_reason(row: ConcreteInput) -> str | None:
    if (
        row.case_id == "ftimage.FT_GLYPH_FORMAT_PLOTTER.source_emitter_inventory"
        and row.operation == "ftimage.glyph_format_emitter_inventory"
    ):
...
def ftimage_outline_owner_real_parity_reason(row: ConcreteInput) -> str | None:
    if row.case_id != "ftimage.FT_OUTLINE_OWNER.destruction_ownership_behavior":
        return None
    if ftimage_subsystem_pending_reason(row):
        return None
    return (
        "direct pinned C, Rust FFI, C ABI, and WASM FT_Outline_New/Done "
        "allocator-event and caller-owned control route"
    )
...
def freetype_core_subsystem_pending_reason(row: ConcreteInput) -> str | None:
    ...
    if row.case_id == "freetype.FT_LOAD_SVG_ONLY.svg_only_behavior":
        ...
        if exact_error_public_route(row.operation, row.case_id, row.expect_error):
            return None
        return (
            "FT_LOAD_SVG_ONLY runtime parity declares input/fonts/DejaVuSans.ttf, "
    ...
    if row.case_id == "freetype.FT_Parameter.tag_data_parameters_match_c_behavior":
        ...
        if exact_error_public_route(row.operation, row.case_id, row.expect_error):
            return None
        return (
            "FT_Parameter runtime parity declares input/fonts/DejaVuSans.ttf "
...
def stream_subsystem_pending_reason(row: ConcreteInput) -> str | None:
    ...
    if row.case_id == "ftlzw.FT_Stream_OpenLZW.opens_valid_lzw_stream":
        ...
        if exact_error_public_route(row.operation, row.case_id, row.expect_error):
            return None
        return (
            "FT_Stream_OpenLZW success declares "
            "compressed/lzw/small-streams.json, but that maintained LZW "
            "payload manifest is absent; exact same-input "
    ...
    if row.case_id == "ftsystem.FT_Memory.custom_allocator_runtime_events":
        if (
            row.operation == "ftsystem.new_library_with_custom_memory"
            and row.params.get("runtime_route") == "actual_custom_memory_lifecycle"
        ):
            return None
        if exact_error_public_route(row.operation, row.case_id, row.expect_error):
            return None
    ...
    if row.case_id == "ftsystem.FT_Stream.external_stream_runtime_contract":
        if (
            row.operation == "ftsystem.external_stream_runtime"
            and row.params.get("runtime_route")
            == "actual_external_stream_runtime"
            and unresolved_assets_reason(row) is None
        ):
            return None
        if exact_error_public_route(row.operation, row.case_id, row.expect_error):
...
def otvalid_unresolved_asset_pending_reason(row: ConcreteInput) -> str | None:
    ...
    if (
        ...
        and not (FIXTURE_DIR / "input/fonts/opentype/valid-all-layout.otf").is_file()
    ):
        return (
            "font references missing fixture "
            "input/fonts/opentype/valid-all-layout.otf; "
...
def fixture_reference_exists(reference: str) -> bool:
    return (FIXTURE_DIR / reference).is_file()


def pending_route_reason(row: ConcreteInput) -> str | None:
    if runtime_id(row) in PINNED_C_SUCCESS_FOR_DECLARED_ERROR_CASES:
        return (
            "the maintained input and route return success in pinned FreeType "
            "2.14.3, so they do not enter the declared error branch. Matching "
            "success in Fontdone is useful control behavior but cannot count "
            "as exact evidence for an expected-error contract; a corrected "
            "error-triggering fixture or controlled facade is required"
        )
    if (
        row.case_id
        ...
        return (
            "pinned FreeType only defines the null-library error for "
            "FT_Done_FreeType; every non-null FT_Library is dereferenced before "
            "destruction, so an arbitrary foreign or poisoned pointer has no "
            "defined C error result. The existing oracle fallback returns a "
            "generic error without entering FT_Done_FreeType and therefore "
            "cannot count as C-contract parity; retain this row only as a "
            "Fontdone safety-policy case until its non-contract status is "
            "represented separately"
        )
    optional_sfnt_table_success_variants = {
        "cmap-record-array-overflow",
        "cmap-short-header",
        "name-langtag-count-missing",
        "name-langtag-record-array-overflow",
        "name-langtag-string-out-of-range",
        "name-record-array-overflow",
        "name-short-header",
    }
    if (
        row.case_id
        == "freetype.FT_New_Memory_Face.error_bad_size_or_unknown_format"
        and row.variant_id in optional_sfnt_table_success_variants
    ):
        return (
            f"pinned FreeType opens the {row.variant_id} fixture successfully "
            "because malformed optional cmap/name data does not make "
            "FT_New_Memory_Face fail; Rust FFI, C ABI, and WASM also open it "
            "successfully. The maintained row still declares an error, so it "
            "cannot count as exact error or function-semantic evidence until "
            "the contract case is replaced by an oracle-backed success "
            "assertion"
        )
    if row.case_id in {
        "ftcache.FTC_SBitCache_Lookup.clears_outputs_before_lookup",
        ...
        if (
            row.params.get("runtime_route") == "actual_ftc_manager_sbit_cache"
            and unresolved_assets_reason(row) is None
        ):
            return None
        return (
            "the maintained route loads and renders a glyph directly instead "
            "of constructing an FTC_Manager/FTC_SBitCache and calling "
            "FTC_SBitCache_Lookup; pinned C therefore returns success from the "
            "facade without exercising the requested null-output or "
            "output-clearing branch. Exact evidence requires a real cache "
            "manager, face requester, cache lookup, and post-call output "
            "capture in every runtime lane"
        )
    if (
        row.case_id
        == "ftglyph.FT_Glyph_To_Bitmap.error_render_failure_preserves_original"
    ):
        destroy_values = row.params.get("destroy_values")
        origin = row.params.get("origin")
        if (
            row.params.get("forced_renderer_failure") is True
            and isinstance(destroy_values, list)
            and {False, True} <= set(destroy_values)
            ...
            return None
        return (
            "the render-failure route requires a maintained font, an explicit "
            "failure trigger, both destroy values, and an x/y origin; without "
            "that complete input it cannot prove caller-handle preservation "
            "and cleanup through pinned C, Rust FFI, C ABI, and WASM ABI"
        )
    ...
    if (
        row.case_id == "ftcache.FTC_SBitCache_Lookup.missing_bitmap_has_null_buffer"
        and row.params.get("runtime_route") == "actual_ftc_manager_sbit_cache"
        and isinstance(row.assets.get("font"), dict)
        and row.assets["font"].get("path")
        == "input/fixtures/assets/fonts/sbit_missing_bitmap.ttf"
    ):
        return None
    ...
    if row.case_id == "ftglyph.FT_Get_Glyph.error_unsupported_format_or_bad_slot_payload":
        slot_formats = row.params.get("slot_formats")
        if (
            isinstance(slot_formats, list)
            and slot_formats
            == [
                "unsupported_tag",
...
def otvalid_real_parity_reason(row: ConcreteInput) -> str | None:
    if (
        row.operation == "ftotval.open_type_validate_then_free"
        and row.case_id
        == "ftotval.FT_OpenType_Free.frees_validated_table_with_face_memory"
        and has_runtime_asset(row)
    ):
        return (
            "FT_OpenType_Free validates a non-null table allocated and released "
            "through the exact live face FT_Memory callbacks in pinned C and "
            "the C ABI, with face-owned release evidence in Rust and WASM"
        )
    if (
...
def focused_success_real_parity_reason(row: ConcreteInput) -> str | None:
    ...
    if row.case_id in svg_glyph_cases:
        return (
            "OpenType SVG glyph document bytes, size metrics, glyph range, "
            "root advance, deep-copy ownership, and accumulated transform/delta "
    ...
    if (
        ...
        and row.params.get("subpixel_rendering_branch") == "defined"
    ):
        return (
            "Subpixel-rendering-enabled LCD errors and stored filter state "
            "validate through an isolated pinned C build, Rust FFI, "
            "independently linked C ABI, and WASM host; the stored five-byte "
            "snapshots prove presets, preservation, and copy boundaries"
        )
    if row.case_id in {
        "ftcolor.FT_Palette_Data_Get.error_color_layers_disabled",
        "ftcolor.FT_Palette_Select.error_color_layers_disabled",
        ...
        return (
            "Color-layer-disabled palette stubs validate exact unconditional "
            "Unimplemented_Feature results and caller-output preservation "
            "through an isolated pinned C build, Rust FFI, independently "
            "linked C ABI, and WASM host"
        )
    ...
    if (
        ...
        return (
            "GX/AAT and classic-kern selection, table bytes, table-length "
            "bounds, dialect behavior, output lifetime, and exact face-memory "
            "release validate through pinned C oracle, Rust FFI, C ABI, "
            "independently linked external C, and WASM ABI"
        )
    if (
        row.operation == "ftlzw.stream_open_lzw"
        and row.case_id == "ftlzw.FT_Stream_OpenLZW.opens_valid_lzw_stream"
        and unresolved_assets_reason(row) is None
    ):
        return (
            "FT_Stream_OpenLZW enabled-build open, callback-backed field "
            "classes, exact decoded byte ranges, backward seek, and wrapper "
            "close ownership validate through pinned C oracle, Rust FFI, C "
            "ABI, and WASM ABI"
        )
    if (
    ...
    if (
        ...
        return (
            ...
            "FT_New_Memory_Face stream base/size/pos/cursor/limit nullness, "
            "exact caller-range identity, caller-buffer survival after "
            "FT_Done_Face, and declared frame byte reads through pinned C "
            "oracle, Rust FFI, C ABI, and WASM ABI"
        )
    if (
    ...
    if (
        ...
        in {
            ...
            "t1tables.FT_Get_PS_Font_Private.cid_keyed_cff_invalid_argument",
            "t1tables.FT_Get_PS_Font_Private.type42_invalid_argument",
        }
        and unresolved_assets_reason(row) is None
    ):
        if row.expect_error:
            if row.case_id == "t1tables.FT_Get_PS_Font_Private.type42_invalid_argument":
                return (
                    "FT_Get_PS_Font_Private Type42 unsupported-service "
                    "Invalid_Argument validates through pinned C oracle, Rust "
                    "FFI, C ABI, and WASM ABI"
                )
            if (
                row.case_id
                ...
                return (
                    "FT_Get_PS_Font_Private CID-keyed CFF unsupported-service "
                    "Invalid_Argument validates through pinned C oracle, Rust "
                    "FFI, C ABI, and WASM ABI"
                )
            return (
    ...
    if (
        ...
        in {
            ...
            "t1tables.FT_Get_PS_Font_Info.cid_keyed_cff_success",
            "t1tables.FT_Get_PS_Font_Info.type42_success",
        ...
        if row.case_id == "t1tables.FT_Get_PS_Font_Info.cid_keyed_cff_success":
            return (
                "FT_Get_PS_Font_Info CID-keyed CFF FontInfo output validates through "
                "pinned C oracle, Rust FFI, C ABI, and WASM ABI"
            )
        if row.case_id == "t1tables.FT_Get_PS_Font_Info.type42_success":
            return (
                "FT_Get_PS_Font_Info Type42 FontInfo output validates through pinned "
                "C oracle, Rust FFI, C ABI, and WASM ABI"
            )
        return (
    ...
    if (
        ...
        in {
            ...
            "t1tables.FT_Has_PS_Glyph_Names.type42_true",
        ...
        if row.case_id == "t1tables.FT_Has_PS_Glyph_Names.type42_true":
            return (
                "FT_Has_PS_Glyph_Names Type42 service behavior validates "
                "through pinned C oracle, Rust FFI, C ABI, and WASM ABI"
            )
    ...
    if (
        ...
        return (
            "FT_Get_BDF_Property SFNT BDF-table property rowset validates the "
            "selected embedded-bitmap strike plus FAMILY_NAME atom and PIXEL_SIZE "
            "integer outputs through pinned C oracle, Rust FFI, C ABI, and WASM ABI"
        )
    if (
        row.operation == "ftbdf.get_bdf_property"
        and row.case_id
        ...
        return (
            "FT_Get_BDF_Property PCF property rowset validates the FAMILY_NAME "
            "atom and signed POINT_SIZE integer through pinned C oracle, Rust "
            "FFI, C ABI, and WASM ABI"
        )
    if (
    ...
    case_reasons = {
        ...
        "freetype.FT_Parameter.wws_only_typographic_name_params_match_c": "FT_Parameter typographic family/subfamily ignore tags validate the OS/2 WWS-only selection path through pinned C oracle, Rust FFI, C ABI, and WASM ABI",
...
def future_batch_real_parity_reason(row: ConcreteInput) -> str | None:
    ...
    if (
        ...
        and row.params.get("runtime_route") == "actual_raster_lifecycle"
    ):
        return (
            "FT_Raster lifecycle registers an actual callback-backed renderer "
            "and validates raster_new, reset(NULL,0), renderer/raster set-mode, "
            "renderer/raster render, raster_done, module_done, exact handle "
            "identity, and teardown ordering through pinned C oracle, Rust "
            "FFI, C ABI, and WASM ABI"
        )
    if (
        row.operation == "ftrender.render_mode_acceptance"
        and row.case_id
        == "ftrender.FT_Renderer_Class.render_mode_acceptance_matches_callbacks"
        and row.params.get("runtime_route") == "actual_renderer_mode_acceptance"
        and unresolved_assets_reason(row) is None
    ):
        return (
            "FT_Renderer_Class mode acceptance invokes the actual smooth, "
            "raster1, sdf, and ot-svg renderer callbacks for every public "
            "FreeType 2.14.3 render mode and compares exact error, post-call "
            "slot format, bitmap descriptor, and bitmap bytes through pinned "
            "C oracle, Rust FFI, C ABI, and WASM ABI"
        )
    ...
    if (
        row.operation == "ftsystem.new_library_with_custom_memory"
        and row.case_id == "ftsystem.FT_Memory.custom_allocator_runtime_events"
        and row.params.get("runtime_route") == "actual_custom_memory_lifecycle"
    ):
        return "FT_Memory custom allocator identity, phase coverage, block ownership, and balanced final destruction validate through pinned C oracle, Rust FFI, C ABI, and WASM ABI"
    if (
        row.operation == "ftcache.manager_ownership"
        and row.case_id == "ftcache.FTC_Manager.owns_faces_sizes_and_cache_nodes"
        and row.params.get("runtime_route") == "actual_ftc_manager_ownership"
    ):
        return "FTC_Manager face, size, cache, node, reset, and destruction ownership validates through pinned C oracle, Rust FFI, C ABI, and WASM ABI"
    if (row.operation, row.case_id) in FTERRDEF_EXACT_ERROR_BATCH:
        return "fterrdef exact error validates through pinned C oracle, Rust FFI, C ABI, and WASM ABI"
    if (
    ...
    case_reasons = {
        ...
        "ftpfr.FT_Get_PFR_Metrics.pfr_metrics_success": "PFR outline resolution, metrics resolution, and before/after-size scales validate through pinned C oracle, Rust FFI, C ABI, and WASM ABI",
    }
    return case_reasons.get(row.case_id)
...
def lifecycle_null_real_parity_reason(row: ConcreteInput) -> str | None:
    ...
    if (
        row.operation == "fterrdef.open_face_non_driver"
        and row.case_id
        == "fterrdef.FT_Err_Invalid_Handle.generic_object_handle_validation"
    ):
        return (
            "FT_Open_Face with FT_OPEN_DRIVER and a registered non-driver "
            "module validates exact Invalid_Handle and null face output "
            "through the pinned C oracle, Rust FFI, C ABI, and WASM ABI"
        )
    if (
    ...
    if (
        ...
        return "FT_Get_Glyph unsupported-format error and preserved output sentinel validate through pinned C oracle, Rust FFI, C ABI, and WASM ABI"
    if (
    ...
    if (
        ...
        return "FT_Get_BDF_Charset_ID SFNT BDF-table selected-strike charset strings validate through pinned C oracle, Rust FFI, C ABI, and WASM ABI"
    if (
        row.operation == "ftbdf.get_bdf_charset_id"
        and row.case_id
        == "ftbdf.FT_Get_BDF_Charset_ID.error_sfnt_bdf_without_selected_strike"
    ...
    if (
        ...
        return "FT_Get_Paint malformed centered-skew final-coordinate rejection validates through pinned C oracle, Rust FFI, C ABI, and WASM ABI"
    if (
        row.operation == "ftcolor.get_paint_malformed"
        and row.case_id
        == "ftcolor.FT_Get_Paint.malformed_transform_payload_reads_return_false"
    ...
    if (
        ...
        return "FT_Get_Paint malformed ColorLine rejection validates through pinned C oracle, Rust FFI, C ABI, and WASM ABI"
    if (
        row.operation == "ftcolor.get_paint_malformed"
        and row.case_id
        == "ftcolor.FT_Get_Paint.malformed_layer_list_index_returns_false"
    ...
    if (
        row.operation == "ftglyph.custom_glyph_lifecycle"
        and row.case_id == "ftglyph.FT_New_Glyph.success_renderer_supported_custom_format"
        and row.params.get("runtime_route") == "actual_custom_glyph_lifecycle"
    ):
        return "FT_New_Glyph custom-renderer selection, class identity, zeroed allocation, caller ownership transfer, and FT_Done_Glyph class destruction validate through pinned C oracle, Rust FFI, C ABI, and WASM ABI"
    if (
    ...
    if (
        ...
        return (
            "FT_Stream_OpenLZW disabled-build argument-independent "
            "Unimplemented_Feature behavior validates through an isolated "
            "pinned FreeType build and isolated no-default-feature Fontdone "
            "Rust, external C artifact, and WASM host builds"
        )
    if (
        row.operation == "ftotval.open_type_validate"
    ...
    if (
        ...
        return "FT_Set_MM_Blend_Coordinates Type 1 MM glyph output after active blend validates FT_Load_Glyph/FT_Render_Glyph through pinned C oracle, Rust FFI, C ABI, and WASM ABI"
    if (
    ...
    if (
        row.operation == "ftsystem.external_stream_runtime"
        and row.case_id == "ftsystem.FT_Stream.external_stream_runtime_contract"
        and row.params.get("runtime_route") == "actual_external_stream_runtime"
        and unresolved_assets_reason(row) is None
    ):
        return (
            "FT_OPEN_STREAM callback-backed valid, seek-failure, and short-"
            "header scenarios validate caller stream identity, read/seek "
            "observation, close-after-I/O ordering, exactly-once close on "
            "success and failure, and caller-owned state survival through "
            "pinned C oracle, Rust FFI, C ABI, and WASM ABI"
        )
    if (
    ...
    if (
        ...
        return "FT_OPEN_STREAM valid caller-owned external memory stream validates return code, opened flag, external-stream face flag, face-to-stream pointer identity, close-callback stream identity and count, and caller stream lifetime through pinned C oracle, Rust FFI, C ABI, and WASM ABI; malformed callback stream harness rows remain pending"
    if (
...
def route_category(row: ConcreteInput) -> tuple[str, str]:
    if row.contract_scope == "fontdone_safety_extension":
        return (
            "safety-extension",
            "Fontdone rejects this undefined or memory-unsafe C input without "
            "dereferencing it; the behavior is verified by package tests but "
            "is explicitly excluded from pinned-FreeType parity numerators and "
            "denominators",
        )
    ...
    unresolved = unresolved_assets_reason(row)
    if unresolved and operation_is_real_parity(row.operation):
        return (
            "pending-route",
            f"{unresolved}; exact runtime parity requires the declared same input "
            "for pinned C, Rust FFI, thin C ABI, and WASM ABI; substituting another "
            "input would be a green placeholder",
        )
    ...
    ftimage_raster_class_real = ftimage_raster_class_real_parity_reason(row)
    if ftimage_raster_class_real:
        return ("real-parity", ftimage_raster_class_real)
...
def build_function_coverage(
    items: dict[str, ManifestSubject], rows: list[dict[str, object]]
) -> dict[str, object]:
    functions = sorted(
        (subject for subject in items.values() if subject.kind == "function"),
        key=lambda subject: (subject.symbol, subject.subject_id),
    )
    total = len(functions)
    coverage: dict[str, object] = {
        "expected_total": EXPECTED_PUBLIC_FUNCTION_COUNT,
        "evidence_categories": sorted(RUNTIME_FUNCTION_EVIDENCE_CATEGORIES),
    }
    for frontend in PARITY_FRONTENDS:
        evidence_subjects = {
            str(row["subject"])
            for row in rows
            if frontend in row["runtime_frontends"]
        }
        missing = [
            {
            ...
            for subject in functions
            if subject.subject_id not in evidence_subjects
        ]
        covered_count = total - len(missing)
        percentage = 100.0 if total == 0 else covered_count * 100.0 / total
        coverage[frontend] = {
...
def build_route_audit(items: dict[str, ManifestSubject]) -> dict[str, object]:
        ...
        rows.append(
            {
                ...
                "reason": reason,
                "runtime_frontends": (
                    list(PARITY_FRONTENDS)
                    if category in RUNTIME_FUNCTION_EVIDENCE_CATEGORIES
                    else []
                ),
    ...
    safety_extension_rows: list[str] = []
    for row in rows:
        ...
        if row["category"] == "safety-extension":
            safety_extension_rows.append(row["runtime_id"])
    ...
    return {
        ...
        "function_coverage": build_function_coverage(items, rows),
        "goal_ledger": {
        ...
        "safety_extensions": {
            "total": len(safety_extension_rows),
            "rows": sorted(safety_extension_rows),
            "note": (
                "These Fontdone hardening behaviors cover inputs for which pinned "
                "FreeType has no defined result. Package tests verify them; they "
                "are not C-contract parity evidence and do not alter pinned "
                "contract denominators."
            ),
        },
...
def write_route_audit(report: dict[str, object], json_path: Path, md_path: Path) -> None:
    ...
    assert isinstance(function_coverage, dict)
    lines.extend(
        [
            "",
            "## Public Function Runtime Coverage",
            "",
            "| Frontend | Covered | Total | Coverage |",
            "|---|---:|---:|---:|",
        ]
    )
    for frontend in PARITY_FRONTENDS:
        coverage = function_coverage[frontend]
    ...
    lines.extend(
        [
            "",
            "A function counts only when at least one manifest case is classified "
            "`real-parity` or `real-null-validation`. Both categories require "
            "the same input to execute against the pinned C oracle, Rust FFI, "
            "C ABI, and Wasm ABI lanes.",
        ]
    )
    rust_coverage = function_coverage["rust"]
    assert isinstance(rust_coverage, dict)
    missing_functions = rust_coverage["missing"]
    assert isinstance(missing_functions, list)
    if missing_functions:
        lines.extend(["", "### Functions Without Runtime Evidence", ""])
        for missing in missing_functions:
            assert isinstance(missing, dict)
            lines.append(f"- `{missing['symbol']}` (`{missing['subject']}`)")
    ...
    safety_extensions = report["safety_extensions"]
    assert isinstance(safety_extensions, dict)
    safety_rows = safety_extensions["rows"]
    assert isinstance(safety_rows, list)
    if safety_rows:
        lines.extend(
            [
                "",
                "## Fontdone Safety Extensions",
                "",
                str(safety_extensions["note"]),
                "",
            ]
        )
        for runtime_id in safety_rows:
            lines.append(f"- `{runtime_id}`")
...
def main() -> int:
    ...
    if args.route_audit:
        ...
        assert isinstance(function_coverage, dict)
        frontend_coverage = {
            frontend: function_coverage[frontend] for frontend in PARITY_FRONTENDS
        }
        for coverage in frontend_coverage.values():
            assert isinstance(coverage, dict)
        print(
            "function coverage "
            + " ".join(
                "{}={}/{}".format(
                    frontend,
                    frontend_coverage[frontend]["covered"],
                    frontend_coverage[frontend]["total"],
                )
                for frontend in PARITY_FRONTENDS
            )
        )
        if not selected:
            incomplete = []
            for frontend, coverage in frontend_coverage.items():
                missing = coverage["missing"]
                assert isinstance(missing, list)
                total = coverage["total"]
                if total != EXPECTED_PUBLIC_FUNCTION_COUNT or missing:
                    incomplete.append(
                        f"{frontend}:total={total}:missing={len(missing)}"
                    )
            if incomplete:
                print(
                    "public function runtime coverage is incomplete: "
                    f"expected={EXPECTED_PUBLIC_FUNCTION_COUNT} "
                    + " ".join(incomplete),
                    file=sys.stderr,
                )
                return 1
    ...
    if errors:
        for error in errors:
            print(error)
        return 1
    input_file_count = len(list(INPUT_DIR.rglob("*.json")))
    print(
        f"checked {len(items)} public API subjects in "
        f"{input_file_count} input files"
    )
    return 0
