...
/// Requested glyph rasterization mode.
pub enum RenderMode {
    /// 8-bit grayscale coverage.
    Normal,
    /// One-bit packed monochrome coverage.
    Mono,
    /// Horizontal RGB subpixel coverage.
    Lcd,
    /// Vertical RGB subpixel coverage.
    LcdV,
    /// Signed-distance-field output.
    Sdf,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
/// Storage interpretation for a rendered bitmap.
pub enum PixelMode {
    /// 8-bit grayscale coverage.
    Gray,
    /// One-bit packed coverage.
    Mono,
    /// Two-bit grayscale values.
    Gray2,
    /// Four-bit grayscale values.
    Gray4,
    /// Horizontal LCD triplets.
    Lcd,
    /// Vertical LCD triplets.
    LcdV,
    /// Four-byte blue/green/red/alpha pixels.
    Bgra,
}

impl PixelMode {
    /// Return the FreeType-compatible `num_grays` value.
...
/// Owned rendered bitmap and FreeType-compatible placement metadata.
pub struct RenderedBitmap {
    /// Logical bitmap width in pixels or subpixels, depending on mode.
    pub width: u32,
    /// Number of stored rows.
    pub rows: u32,
    /// Signed row stride in bytes.
    pub pitch: i32,
    /// Buffer storage interpretation.
    pub pixel_mode: PixelMode,
    /// Number of representable gray levels.
    pub num_grays: u16,
    /// Horizontal placement offset in pixels.
    pub left: i32,
    /// Top placement coordinate in pixels.
    pub top: i32,
    /// Owned bitmap bytes; addressable length is `abs(pitch) * rows`.
    pub buffer: Vec<u8>,
}
...
impl Font {
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] when the selected glyph cannot be loaded, hinted,
    /// or rasterized.
    ...
    /// Render one Unicode scalar using a FreeType render mode.
    ///
    /// # Errors
    ///
...
/// Return FreeType's two-byte-aligned monochrome pitch for `width` pixels.
...
/// # Panics
///
/// Debug builds can panic on arithmetic overflow. `alignment` must be a
/// non-zero power of two.
pub fn pad_ceil(value: usize, alignment: usize) -> usize {
    (value + alignment - 1) & !(alignment - 1)
}

/// Expand one most-significant-bit-first monochrome row to 8-bit coverage.
///
/// Missing source bytes are interpreted as zero coverage.
