...
#[derive(Clone)]
pub struct Font {
    /// Shared parsed font bytes and tables.
    pub data: Arc<FontData>,
    /// Active typographic point size.
    pub size_pt: f32,
    ...
    sfnt_bdf: Option<SfntBdfTable>,
    cid_type1: Option<CidType1Metadata>,
    pfr: Option<PfrFont>,
...
#[derive(Debug, Clone, PartialEq, Eq)]
struct SfntBdfTable {
    strikes: Vec<SfntBdfStrike>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct SfntBdfStrike {
    ppem: u16,
    properties: Vec<BdfPropertyEntry>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct CidType1Metadata {
    registry: String,
    ordering: String,
    supplement: i32,
    cid_count: u16,
...
enum FaceKind {
    ...
    Pcf,
    Pfr,
    CidType1 { is_fixed_pitch: bool },
    Type1 { is_fixed_pitch: bool },
    Type42 { is_fixed_pitch: bool },
...
pub struct WinFntHeader {
    ...
    pub version: u16,
    /// Total bytes in the FNT resource.
    pub file_size: u32,
    /// Fixed-width copyright field as stored in the resource.
    pub copyright: [u8; 60],
    /// FNT type flags.
    pub file_type: u16,
    /// Nominal point size.
    ...
    pub ascent: u16,
    /// Leading included inside the character cell.
    pub internal_leading: u16,
    /// Leading added outside the character cell.
    pub external_leading: u16,
    /// Nonzero when the strike is italic.
    pub italic: u8,
    /// Nonzero when the strike is underlined.
    pub underline: u8,
    /// Nonzero when the strike is struck out.
    pub strike_out: u8,
    /// Windows font-weight value.
    pub weight: u16,
    /// Windows character-set identifier.
    pub charset: u8,
    /// Fixed character width, or zero for a proportional strike.
    pub pixel_width: u16,
    /// Character-cell height in pixels.
    pub pixel_height: u16,
    /// Packed pitch and family classification.
    ...
    pub last_char: u8,
    /// Default-character offset relative to `first_char`.
    pub default_char: u8,
    /// Word-break character offset relative to `first_char`.
    pub break_char: u8,
    ...
    /// Historical in-memory bitmap pointer field.
    pub bits_pointer: u32,
    /// Offset of bitmap data in the resource.
    pub bits_offset: u32,
    /// Reserved header byte.
    pub reserved: u8,
    /// Extended FNT flags.
    pub flags: u32,
    /// Global A-space for ABC-width strikes.
    pub a_space: u16,
    /// Global B-space for ABC-width strikes.
    pub b_space: u16,
    /// Global C-space for ABC-width strikes.
    pub c_space: u16,
    /// Offset of the color table, when present.
    pub color_table_offset: u32,
    /// Reserved extended-header words.
    pub reserved1: [u64; 4],
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct WinFntBitmapSize {
...
fn winfnt_bitmap_size(header: WinFntHeader) -> WinFntBitmapSize {
    ...
    // `FT_PIX_ROUND`, and oversized-y-ppem correction exactly so size
    // selection and `FT_Face::available_sizes` use one contract.
    let mut size = i32::from(header.nominal_point_size) << 6;
    let x_resolution = i32::from(if header.horizontal_resolution == 0 {
        72
    } else {
        header.horizontal_resolution
    });
    let y_resolution = i32::from(if header.vertical_resolution == 0 {
        72
    } else {
        header.vertical_resolution
    });
    let mut y_ppem = ft_pix_round(ft_mul_div(size, y_resolution, 72));
    let pixel_height = i32::from(header.pixel_height) << 6;
    if y_ppem > pixel_height {
        y_ppem = pixel_height;
        size = ft_mul_div(y_ppem, 72, y_resolution);
    }
    let x_ppem = ft_pix_round(ft_mul_div(size, x_resolution, 72));

    WinFntBitmapSize {
        height: i16_from_i32(i32::from(header.pixel_height) + i32::from(header.external_leading)),
        width: i16_from_i32(i32::from(header.avg_width)),
        size,
        x_ppem,
        y_ppem,
    }
...
const PCF_FILE_VERSION: u32 = 0x7063_6601;
...
const PCF_COMPRESSED_METRICS: u32 = 0x0000_0100;
const PCF_FORMAT_MASK: u32 = 0xffff_ff00;
...
fn pcf_i16(data: &[u8], offset: usize, msb: bool) -> Option<i16> {
    pcf_u16(data, offset, msb).map(|value| i16::from_ne_bytes(value.to_ne_bytes()))
}

fn pcf_u32(data: &[u8], offset: usize, msb: bool) -> Option<u32> {
...
fn pcf_i32(data: &[u8], offset: usize, msb: bool) -> Option<i32> {
    pcf_u32(data, offset, msb).map(|value| i32::from_ne_bytes(value.to_ne_bytes()))
}
...
fn pcf_tables(data: &[u8]) -> Result<Vec<(u32, PcfTable)>, FontError> {
    ...
    tables.sort_by_key(|(_, table)| table.offset);
    if tables
        .windows(2)
...
fn pcf_table(tables: &[(u32, PcfTable)], table_type: u32) -> Result<PcfTable, FontError> {
    tables
        .iter()
...
fn parse_pcf_properties(data: &[u8], table: PcfTable) -> Result<Vec<BdfPropertyEntry>, FontError> {
    ...
    for index in 0..count {
        ...
        let value = if is_string {
            ...
            // FreeType's PCF service reports every non-string property as a
            // signed BDF_PROPERTY_TYPE_INTEGER, regardless of its BDF name.
            BdfPropertyValue::Integer(raw_value)
        };
        let atom_c_string = match &value {
            BdfPropertyValue::Atom(atom) => CString::new(atom.as_str()).ok(),
            BdfPropertyValue::Integer(_) | BdfPropertyValue::Cardinal(_) => None,
        };
        properties.push(BdfPropertyEntry {
            name,
            value,
            atom_c_string,
        });
...
fn parse_sfnt_bdf_table(data: &[u8]) -> Option<SfntBdfTable> {
    ...
    fn string_at(strings: &[u8], offset: u32) -> Option<&str> {
        let tail = strings.get(usize::try_from(offset).ok()?..)?;
    ...
    let strike_count = usize::from(u16_at(data, 2)?);
    let strings_offset = usize::try_from(u32_at(data, 4)?).ok()?;
    if strings_offset < 8usize.checked_add(strike_count.checked_mul(4)?)?
        || strings_offset >= data.len()
    {
        return None;
    }
    let strings = data.get(strings_offset..)?;
    let mut property_cursor = 8usize.checked_add(strike_count.checked_mul(4)?)?;
    let mut strikes = Vec::with_capacity(strike_count);
    for strike_index in 0..strike_count {
        let directory_offset = 8usize.checked_add(strike_index.checked_mul(4)?)?;
        let ppem = u16_at(data, directory_offset)?;
        let property_count = usize::from(u16_at(data, directory_offset + 2)?);
        let property_bytes = property_count.checked_mul(10)?;
        if property_cursor.checked_add(property_bytes)? > strings_offset {
            return None;
        }
        let mut properties = Vec::with_capacity(property_count);
        for property_index in 0..property_count {
            let offset = property_cursor.checked_add(property_index.checked_mul(10)?)?;
            let name = string_at(strings, u32_at(data, offset)?)?;
            let property_type = u16_at(data, offset + 4)?;
            if property_type & 0x10 == 0 {
                continue;
            }
            let raw_value = u32_at(data, offset + 6)?;
            let value = match property_type & 0x0F {
                0 | 1 => BdfPropertyValue::Atom(string_at(strings, raw_value)?.to_string()),
                2 => BdfPropertyValue::Integer(i32::from_be_bytes(raw_value.to_be_bytes())),
                3 => BdfPropertyValue::Cardinal(raw_value),
                _ => continue,
            };
            let atom_c_string = match &value {
                BdfPropertyValue::Atom(atom) => CString::new(atom.as_str()).ok(),
        ...
        strikes.push(SfntBdfStrike { ppem, properties });
        property_cursor += property_bytes;
    }
    Some(SfntBdfTable { strikes })
}
...
fn parse_bdf_metadata(text: &str) -> Result<BdfMetadata, FontError> {
    ...
    let unicode_charmap = property_atom("CHARSET_REGISTRY")
        .zip(property_atom("CHARSET_ENCODING"))
        .is_some_and(|(registry, encoding)| {
...
fn parse_pcf_metadata(data: &[u8]) -> Result<BdfMetadata, FontError> {
    ...
    let family_name = property_atom("FAMILY_NAME").unwrap_or("PCF").to_string();
    let unicode_charmap = property_atom("CHARSET_REGISTRY")
        .zip(property_atom("CHARSET_ENCODING"))
        .is_some_and(|(registry, encoding)| {
            let registry = registry.to_ascii_lowercase();
    ...
    let pixel_width = right.saturating_sub(left).max(advance).max(1);
    let pixel_height = font_ascent
        .saturating_add(font_descent)
        .max(metric_ascent.saturating_add(metric_descent))
        .max(1);
...
fn bdf_font_data(data: &[u8], size_pt: f32, metadata: &BdfMetadata) -> Arc<FontData> {
    ...
    let font_data = Arc::new(FontData {
        ...
        cmap: tt::cmap::CmapTable::from_sparse_mapping(
            if metadata.unicode_charmap { 3 } else { 0 },
            if metadata.unicode_charmap { 1 } else { 0 },
        ...
        head: tt::head::HeadTable {
            ...
            // nonzero synthetic UPEM only so shared size-metric helpers avoid
            // division by zero; their FaceKind keeps SCALABLE clear.
        ...
        maxp: tt::maxp::MaxpTable {
            // FreeType's BDF driver reserves glyph index zero and exposes
            // every encoded bitmap at index `array_index + 1`.
            num_glyphs: metadata.glyph_count.saturating_add(1),
            ..tt::maxp::MaxpTable::default()
        },
        name: tt::name::NameTable {
            format: 0,
...
fn parse_cid_type1_metadata(
    cleartext: &[u8],
) -> Result<(Type1Metadata, CidType1Metadata), FontError> {
    const CID_HEADER: &[u8] = b"%!PS-Adobe-3.0 Resource-CIDFont";
    if !cleartext.starts_with(CID_HEADER) {
        return Err(FontError::InvalidFont(
            "CID Type 1 resource header is missing".into(),
        ));
    }
    let text = std::str::from_utf8(cleartext)
        .map_err(|_| FontError::InvalidFont("CID Type 1 dictionary is not UTF-8".into()))?;
    if !text.contains("StartData") {
        return Err(FontError::InvalidFont(
            "CID Type 1 StartData is missing".into(),
        ));
    }
    let metadata = parse_type1_metadata(cleartext)?;
    let registry = type1_string_value(text, "Registry")
    ...
    let gd_bytes = type1_i32_value(text, "GDBytes")
        .filter(|value| (1..=4).contains(value))
        .ok_or_else(|| FontError::InvalidFont("CID Type 1 GDBytes is invalid".into()))?;
    let map_entry_size = usize::try_from(fd_bytes + gd_bytes)
        .map_err(|_| FontError::InvalidFont("CID Type 1 map entry size is invalid".into()))?;
    let minimum_map_size = usize::from(cid_count)
        .checked_mul(map_entry_size)
        .ok_or_else(|| FontError::InvalidFont("CID Type 1 map size overflows".into()))?;
    let start_data = text
        .find("StartData")
        .and_then(|offset| offset.checked_add("StartData".len() + 1))
        .ok_or_else(|| FontError::InvalidFont("CID Type 1 StartData offset is invalid".into()))?;
    let hexadecimal_bytes = text[start_data..]
        .bytes()
        .take_while(|byte| *byte != b'>')
        .filter(|byte| byte.is_ascii_hexdigit())
        .count()
        / 2;
    if hexadecimal_bytes < minimum_map_size {
        return Err(FontError::InvalidFont(
            "CID Type 1 CIDMap is truncated".into(),
        ));
    }
    Ok((
        metadata,
...
fn type1_font_info_from_metadata(metadata: &Type1Metadata) -> Type1FontInfo {
    Type1FontInfo {
        ...
        // FreeType's Type 1 and Type 42 dictionaries store `ItalicAngle` as
        // fixed-point in the public `PS_FontInfoRec`.
...
fn type42_sfnt_data(data: &[u8]) -> Option<Vec<u8>> {
    let cleartext = type1_cleartext(data)?;
    let text = std::str::from_utf8(cleartext).ok()?;
    if type1_i32_value(text, "FontType") != Some(42) {
        return None;
    }
    let sfnts = type1_exact_key_tail(text, "sfnts")?;
    let array_start = sfnts.find('[')?;
    let array = &sfnts[array_start + 1..];
    ...
    for byte in array.bytes() {
        match byte {
            ...
            _ if in_hex_string && byte.is_ascii_hexdigit() => {
    ...
    (!in_hex_string && !output.is_empty()).then_some(output)
}
...
fn non_sfnt_outline_font_data(
    ...
    let font_data = Arc::new(FontData {
        ...
        hmtx: tt::hmtx::HmtxTable {
            h_metrics: vec![tt::hmtx::LongHorMetric {
                advance_width: u16::try_from(max_advance.max(0)).unwrap_or(u16::MAX),
                lsb: 0,
            }],
            left_side_bearings: Vec::new(),
        },
...
/// Size-specific OpenType SVG document attached to a loaded glyph slot.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SvgGlyphDocument {
    /// Uncompressed XML document bytes.
    pub document: Vec<u8>,
    /// Active face size metrics captured when the glyph was loaded.
    pub metrics: SizeMetrics,
    /// Font units per EM.
    pub units_per_em: u16,
    /// First glyph covered by this document.
    pub start_glyph_id: u16,
    /// Last glyph covered by this document.
    pub end_glyph_id: u16,
    /// Accumulated 16.16 affine transform `(xx, xy, yx, yy)`.
    pub transform: (i32, i32, i32, i32),
    /// Accumulated 26.6 translation `(x, y)`.
    pub delta: (i32, i32),
}
...
pub enum SizeRequestType {
    /// Nominal body size.
    Nominal,
    /// Real glyph dimension.
    RealDim,
    /// Face bounding-box dimension.
    BBox,
    /// Cell dimension.
    Cell,
    /// Direct 16.16 scale request.
    Scales,
}
...
pub struct SizeRequest {
    /// Algorithm used to interpret width and height.
    pub request_type: SizeRequestType,
    /// Requested width: 26.6 points, or 16.16 scale for [`SizeRequestType::Scales`].
    pub width: i64,
    /// Requested height: 26.6 points, or 16.16 scale for [`SizeRequestType::Scales`].
    pub height: i64,
    /// Horizontal DPI; zero selects the FreeType default.
    pub hori_resolution: u32,
    /// Vertical DPI; zero selects the FreeType default.
    pub vert_resolution: u32,
}

/// Failure while converting a FreeType-style size request to active metrics.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SizeRequestError {
    /// A required scale calculation divided by zero.
    DivideByZero,
    /// The requested pixel dimension is invalid.
    InvalidPixelSize,
    /// The request produces an unsupported pixels-per-EM value.
    InvalidPpem,
    /// The active font driver does not implement this request kind.
    UnimplementedFeature,
}

/// Failure while selecting an embedded bitmap strike.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SelectSizeError {
    /// The face has no selectable fixed-size strikes.
    NoFixedSizes,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FaceInfo {
    /// Number of faces in the input container.
    pub num_faces: usize,
    /// Zero-based active face index.
    pub face_index: usize,
    /// Preferred family name.
    pub family_name: String,
    /// Preferred style/subfamily name.
    pub style_name: String,
    /// Optional PostScript name.
    pub postscript_name: Option<String>,
    /// Driver-format label such as `TrueType`.
    pub font_format: &'static str,
    /// Font units per EM.
    pub units_per_em: u16,
    /// Addressable glyph count.
    pub num_glyphs: u16,
    /// Global face bounding box in font units.
    pub bbox: BBox,
    /// Face ascender in font units.
    pub ascender: i16,
    /// Face descender in font units.
    pub descender: i16,
    /// Recommended line height in font units.
    pub height: i16,
    /// Maximum horizontal advance in font units.
    pub max_advance_width: i32,
    /// Maximum vertical advance in font units.
    pub max_advance_height: i32,
    /// Underline position in font units.
    pub underline_position: i16,
    /// Underline thickness in font units.
    pub underline_thickness: i16,
    /// FreeType-compatible `FT_FACE_FLAG_*` bitset.
    pub face_flags: u32,
    /// FreeType-compatible `FT_STYLE_FLAG_*` bitset.
    pub style_flags: u32,
    /// OS/2 embedding restriction bits.
...
pub struct CharmapInfo {
    /// Zero-based charmap index in the face.
    pub index: usize,
    /// Source-platform identifier.
    pub platform_id: u16,
    /// Platform encoding identifier.
    pub encoding_id: u16,
    /// cmap subtable format.
    pub format: u16,
    /// Format-defined language identifier.
    pub language_id: u32,
}
...
pub struct SfntTableInfo {
    /// Zero-based table-directory index.
    pub index: usize,
    /// Big-endian four-byte SFNT tag.
    pub tag: u32,
    /// Byte offset from the selected SFNT face.
    pub offset: u32,
...
pub struct GlyphMask {
    /// Number of tightly packed pixels per row.
    pub width: u32,
    /// Number of rows.
    pub height: u32,
    /// Owned row-major 8-bit coverage; length is `width * height`.
    pub pixels: Vec<u8>,
...
pub struct GlyphSlotMetrics {
    /// Grid-fitted glyph width in 26.6 pixels.
    pub width: i32,
    /// Grid-fitted glyph height in 26.6 pixels.
    pub height: i32,
    /// Horizontal left bearing in 26.6 pixels.
    pub hori_bearing_x: i32,
    /// Horizontal top bearing in 26.6 pixels.
    pub hori_bearing_y: i32,
    /// Horizontal advance in 26.6 pixels.
    pub hori_advance: i32,
    /// Vertical left bearing in 26.6 pixels.
    pub vert_bearing_x: i32,
    /// Vertical top bearing in 26.6 pixels.
    pub vert_bearing_y: i32,
    /// Vertical advance in 26.6 pixels.
...
pub struct SubGlyphTransform {
    /// Horizontal-to-horizontal 16.16 matrix coefficient.
    pub xx: i32,
    /// Vertical-to-horizontal 16.16 matrix coefficient.
    pub xy: i32,
    /// Horizontal-to-vertical 16.16 matrix coefficient.
    pub yx: i32,
    /// Vertical-to-vertical 16.16 matrix coefficient.
    pub yy: i32,
}
...
pub struct SubGlyphInfo {
    ...
    pub index: u16,
    /// FreeType-compatible composite component flags.
    pub flags: u16,
    /// First component argument; interpretation follows `flags`.
    pub arg1: i32,
    /// Second component argument; interpretation follows `flags`.
    pub arg2: i32,
    /// Component transformation in 16.16 coefficients.
...
impl Font {
    ...
    pub(crate) fn memory_face(
        ...
        if data.starts_with(b"%!PS-Adobe-3.0 Resource-CIDFont") {
            return Self::cid_type1_face(data, face_index, size_pt);
        }
        if type42_sfnt_data(data).is_some() {
            return Self::type42_face(data, face_index, size_pt);
        }
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] when required SFNT data is unsupported, missing,
    /// or malformed.
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] when the container is invalid, the index is out
    /// of range, or the selected face cannot be parsed.
    ...
    fn type1_face(data: &[u8], face_index: usize, size_pt: f32) -> Result<Self, FontError> {
        ...
        let font_data = type1_font_data(data, size_pt, &metadata, 2);
    ...
    fn cid_type1_face(data: &[u8], face_index: usize, size_pt: f32) -> Result<Self, FontError> {
        if face_index != 0 {
            return Err(FontError::InvalidFont(format!(
                "face index {face_index} out of range for 1 face(s)"
            )));
        ...
        let cleartext = type1_cleartext(data)
            .ok_or_else(|| FontError::InvalidFont("missing CID Type 1 dictionary".into()))?;
        let (metadata, cid_type1) = parse_cid_type1_metadata(cleartext)?;
        let type1_font_info = type1_font_info_from_metadata(&metadata);
        let font_data = type1_font_data(data, size_pt, &metadata, cid_type1.cid_count);
        let is_italic = metadata.italic_angle != 0;
        let face_globals = crate::autohint::globals::FaceGlobals::new(font_data.clone(), is_italic);
        let size_metrics = SizeMetrics::from_char_size(
            i32_from_f32((size_pt * 64.0).round()),
    ...
    fn type42_face(data: &[u8], face_index: usize, size_pt: f32) -> Result<Self, FontError> {
        if face_index != 0 {
            return Err(FontError::InvalidFont(format!(
                "face index {face_index} out of range for 1 face(s)"
            )));
        ...
        let cleartext = type1_cleartext(data)
            .ok_or_else(|| FontError::InvalidFont("missing Type 42 dictionary".into()))?;
        let metadata = parse_type1_metadata(cleartext)?;
        ...
        font.type1_font_info = Some(type1_font_info_from_metadata(&metadata));
        font.family_name = metadata.family_name;
        font.subfamily_name = metadata.style_name;
        font.is_italic = metadata.italic_angle != 0;
        Ok(font)
    }
    ...
    fn pfr_face(data: &[u8], face_index: usize, size_pt: f32) -> Result<Self, FontError> {
        ...
        let metadata = Type1Metadata {
            ...
            italic_angle: 0,
            is_fixed_pitch: !pfr.proportional,
            underline_position: -(i16_from_i32(i32::from(pfr.outline_resolution)) / 10),
            underline_thickness: i16_from_i32(i32::from(pfr.outline_resolution) / 30),
            bbox: pfr.bbox,
        };
        let glyph_count = u16::try_from(pfr.advances.len().saturating_add(1))
            .map_err(|_| FontError::InvalidFont("too many PFR glyphs".into()))?;
        let font_data = non_sfnt_outline_font_data(
            data,
            size_pt,
            &metadata,
            glyph_count,
    ...
    fn bdf_face(data: &[u8], face_index: usize, size_pt: f32) -> Result<Self, FontError> {
        ...
        Self::bitmap_face(data, face_index, size_pt, metadata, FaceKind::Bdf)
    }

    fn pcf_face(data: &[u8], face_index: usize, size_pt: f32) -> Result<Self, FontError> {
        let metadata = parse_pcf_metadata(data)?;
        Self::bitmap_face(data, face_index, size_pt, metadata, FaceKind::Pcf)
    }
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] under the same conditions as
    /// [`Self::truetype_face`].
    ...
    fn truetype_face_with_load_mode_and_design_coords(
        ...
        // `sfnt_load_face` keeps the face usable when the cmap service cannot
        // build any charmap records.  The malformed table remains observable
        // as an empty public charmap list instead of aborting face creation.
        let cmap = tt::cmap::parse_cmap(cmap_bytes).unwrap_or_default();
        ...
        let mut name = match dir
            .find(data, tag(b"name"))
            .and_then(|bytes| tt::name::parse_name(bytes).ok())
        {
            Some(table) => table,
        ...
        let cff = dir
            ...
            .transpose()?;
        let cff2 = dir
            .find(data, tag(b"CFF2"))
            .map(tt::cff::parse_cff2)
            .transpose()?;
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] when the bytes are not a valid supported SFNT or
    /// collection.
    ...
    pub(crate) fn has_postscript_glyph_names(&self) -> bool {
        ...
        matches!(
            self.face_kind,
            FaceKind::Type1 { .. } | FaceKind::Type42 { .. }
    ...
    pub(crate) fn pfr(&self) -> Option<&PfrFont> {
        self.pfr.as_ref()
    }
    ...
    pub(crate) fn winfnt_bitmap_size(&self) -> Option<WinFntBitmapSize> {
        match self.face_kind {
    ...
    fn active_bdf_properties(&self) -> Option<&[BdfPropertyEntry]> {
        match self.face_kind {
            FaceKind::Bdf | FaceKind::Pcf => Some(&self.bdf_properties),
            FaceKind::Sfnt => self
                .sfnt_bdf
                .as_ref()?
                .strikes
                .iter()
                .find(|strike| strike.ppem == self.size_metrics.y_ppem)
                .map(|strike| strike.properties.as_slice()),
            FaceKind::CidType1 { .. }
            | FaceKind::Type1 { .. }
            | FaceKind::Type42 { .. }
            | FaceKind::Pfr
            | FaceKind::WinFnt { .. } => None,
        }
    }

    pub(crate) fn has_sfnt_bdf_table(&self) -> bool {
        self.face_kind == FaceKind::Sfnt && self.sfnt_bdf.is_some()
    }
    ...
    pub fn font_format(&self) -> &'static str {
        match self.face_kind {
            ...
            FaceKind::Pcf => return "PCF",
            FaceKind::Pfr => return "PFR",
            FaceKind::CidType1 { .. } => return "CID Type 1",
            FaceKind::Type1 { .. } => return "Type 1",
            FaceKind::Type42 { .. } => return "Type 42",
    ...
    pub(crate) fn cid_for_glyph_index(&self, glyph_index: u32) -> Option<u16> {
        if let Some(cid) = self.cid_type1.as_ref() {
            return (glyph_index < u32::from(cid.cid_count))
                .then(|| u16::try_from(glyph_index).ok())
                .flatten();
        }
        self.data
            .cff
    ...
    pub fn face_flags(&self) -> u32 {
        ...
        if matches!(self.face_kind, FaceKind::Bdf | FaceKind::Pcf) {
            ...
            // horizontal fixed-size strikes.
            return FT_FACE_FLAG_FIXED_SIZES | FT_FACE_FLAG_HORIZONTAL;
        }

        if let FaceKind::Pfr = self.face_kind {
            let Some(pfr) = self.pfr.as_ref() else {
                return 0;
            };
            let mut flags = FT_FACE_FLAG_SCALABLE;
            flags |= if pfr.vertical {
        ...
        if let FaceKind::CidType1 { is_fixed_pitch } = self.face_kind {
            // The naked CID Type 1 driver advertises scalable horizontal
            // outlines, native hinting, and the public CID-keyed face flag.
            let mut flags = FT_FACE_FLAG_SCALABLE
                | FT_FACE_FLAG_HORIZONTAL
                | FT_FACE_FLAG_HINTER
        ...
        if let FaceKind::Type42 { is_fixed_pitch } = self.face_kind {
            // FreeType's Type 42 driver exposes the embedded TrueType outlines
            // through a non-SFNT PostScript wrapper and always provides the
            // PostScript glyph-name service.
            let mut flags = FT_FACE_FLAG_SCALABLE
                | FT_FACE_FLAG_HORIZONTAL
                | FT_FACE_FLAG_GLYPH_NAMES
                | FT_FACE_FLAG_HINTER;
    ...
    pub(crate) fn try_set_char_size(
        ...
        let (mut width, mut height) = normalize_char_size_dimensions(char_width, char_height);
        width = width.max(64);
        height = height.max(64);
        let (x_dpi, y_dpi) = normalize_size_resolutions(x_dpi, y_dpi);
        let size_metrics = match self.face_kind {
            FaceKind::WinFnt { header } => SizeMetrics::from_winfnt_request(
                SizeRequest {
                    request_type: SizeRequestType::Nominal,
                    width: i64::from(width),
                    height: i64::from(height),
                    hori_resolution: x_dpi,
                    vert_resolution: y_dpi,
                },
                header,
            )?,
            _ => SizeMetrics::try_from_char_size(width, height, x_dpi, y_dpi, &self.data)?,
        };
    ...
    pub fn set_pixel_sizes(&mut self, pixel_width: u32, pixel_height: u32) {
        let _ = self.try_set_pixel_sizes(pixel_width, pixel_height);
    }

    pub(crate) fn try_set_pixel_sizes(
        &mut self,
        pixel_width: u32,
        pixel_height: u32,
    ) -> Result<(), SizeRequestError> {
        let (mut width, mut height) = if pixel_width == 0 {
        ...
        let size_metrics = match self.face_kind {
            FaceKind::WinFnt { header } => SizeMetrics::from_winfnt_request(
                SizeRequest {
                    request_type: SizeRequestType::Nominal,
                    width: i64::from(width) << 6,
                    height: i64::from(height) << 6,
                    hori_resolution: 0,
                    vert_resolution: 0,
                },
                header,
    ...
    /// # Errors
    ///
    /// Returns [`SizeRequestError`] when scale computation divides by zero or
    /// produces an invalid pixel size/ppem.
    pub fn request_size(&mut self, request: SizeRequest) -> Result<(), SizeRequestError> {
        self.size_metrics = match self.face_kind {
            FaceKind::WinFnt { header } => SizeMetrics::from_winfnt_request(request, header)?,
            _ => SizeMetrics::from_size_request(request, &self.data)?,
        };
    ...
    /// Select one embedded bitmap strike by zero-based index.
    ///
    /// # Errors
    ///
    ...
    pub fn select_size(&mut self, strike_index: usize) -> Result<(), SelectSizeError> {
        if let FaceKind::WinFnt { header } = self.face_kind {
            if strike_index != 0 {
                return Err(SelectSizeError::InvalidArgument);
            }
            self.size_metrics = SizeMetrics::from_winfnt_strike(header, None);
            self.size_pt = f32::from(self.size_metrics.y_ppem);
            self.data.size_pt.set(self.size_pt);
            sync_active_size_metrics(&self.data, self.size_metrics);
            self.face_globals =
    ...
    /// Synchronize face-owned public autofitter properties before a glyph load.
    ///
    /// FreeType exposes `AF_FaceGlobalsRec::glyph_styles` directly through
    /// `glyph-to-script-map`, so callers can mutate it without another API
    /// call.  The FFI face owns that public array; copy it into the pure-Rust
    /// autofitter at the load boundary together with `increase-x-height`.
    pub(crate) fn sync_autohint_property_state(
        &self,
        glyph_styles: &[u16],
        increase_x_height: u32,
    ) {
        self.face_globals
            .set_property_state(glyph_styles, increase_x_height);
    }
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] for a non-SFNT face, missing table, negative or
    /// overflowing offset, or a read outside the table/input bytes.
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] when the face is not SFNT or `tag` is absent.
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] if any mapped glyph in the text run cannot be
    /// loaded for its advance.
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] when the mapped glyph cannot be loaded, scaled,
    /// or hinted.
    ...
    pub(crate) fn load_svg_glyph(
        &self,
        glyph_index: u16,
    ) -> Result<(GlyphSlotMetrics, SvgGlyphDocument), FontError> {
        let document = self
            .data
            .svg
            .as_ref()
            .and_then(|svg| svg.document_for_glyph(glyph_index))
            .ok_or_else(|| FontError::InvalidArgument("SVG glyph not available".into()))?;
        let metrics = GlyphSlotMetrics {
            width: 0,
            height: 0,
            hori_bearing_x: 0,
            hori_bearing_y: 0,
            hori_advance: self.glyph_index_hori_advance_26dot6(glyph_index),
            vert_bearing_x: 0,
            vert_bearing_y: 0,
            vert_advance: self.glyph_index_vert_advance_26dot6(glyph_index),
        };
        Ok((
            metrics,
            SvgGlyphDocument {
                document: document.document.clone(),
                metrics: self.size_metrics(),
                units_per_em: self.data.head.units_per_em,
                start_glyph_id: document.start_glyph_id,
                end_glyph_id: document.end_glyph_id,
                transform: (0x1_0000, 0, 0, 0x1_0000),
                delta: (0, 0),
            },
        ))
    }
    ...
    fn glyph_slot_load_truetype_no_scale(&self, glyph: u16) -> Result<GlyphSlotLoad, FontError> {
        let outline = tt::glyf::load_glyph_no_hinting(
            &self.data.glyf_data,
            &self.data.loca_data,
...
impl Font {
    ...
    fn slot_load_from_scaled(
        ...
        } else if self.data.has_cff_outlines() {
            // CFF/CFF2 keep made-up vertical metrics mostly inert for horizontal
...
impl SizeMetrics {
    ...
    fn from_size_request(request: SizeRequest, data: &FontData) -> Result<Self, SizeRequestError> {
        ...
        if !data.has_cff_outlines() && (x_ppem == 0 || y_ppem == 0) {
    ...
    fn from_winfnt_request(
        request: SizeRequest,
        header: WinFntHeader,
    ) -> Result<Self, SizeRequestError> {
        let strike = winfnt_bitmap_size(header);
        let requested_height =
            request_dimension(request.height, request.vert_resolution, "height")?;
        let requested_height = (requested_height + 32) >> 6;
        let expected_height = match request.request_type {
            SizeRequestType::Nominal => i64::from((strike.y_ppem + 32) >> 6),
            SizeRequestType::RealDim => i64::from(header.pixel_height),
            SizeRequestType::BBox | SizeRequestType::Cell | SizeRequestType::Scales => {
                return Err(SizeRequestError::UnimplementedFeature);
            }
        };
        if requested_height != expected_height {
            return Err(SizeRequestError::InvalidPixelSize);
        }
        Ok(Self::from_winfnt_strike(header, Some(request)))
    }

    fn from_winfnt_strike(header: WinFntHeader, request: Option<SizeRequest>) -> Self {
        let strike = winfnt_bitmap_size(header);
        let x_ppem = u16::try_from((strike.x_ppem + 32) >> 6).unwrap_or(u16::MAX);
        let y_ppem = u16::try_from((strike.y_ppem + 32) >> 6).unwrap_or(u16::MAX);
        SizeMetrics {
            x_ppem,
            y_ppem,
            x_scale: 1 << 16,
            y_scale: 1 << 16,
            ascender: i32::from(header.ascent) << 6,
            descender: -(i32::from(header.pixel_height) - i32::from(header.ascent)) << 6,
            height: i32::from(strike.height) << 6,
            max_advance: i32::from(header.max_width) << 6,
            x_dpi: request.map_or(72, |request| normalize_dpi(request.hori_resolution)),
            y_dpi: request.map_or(72, |request| normalize_dpi(request.vert_resolution)),
            char_width: request
                .and_then(|request| i32::try_from(request.width).ok())
                .unwrap_or(strike.size),
            char_height: request
                .and_then(|request| i32::try_from(request.height).ok())
                .unwrap_or(strike.size),
        }
    }
...
fn true_type_hint_scales(data: &FontData, metrics: SizeMetrics) -> (i32, i32) {
    if !data.has_cff_outlines() && data.head.flags & 8 != 0 {
