...
mod tests {
    ...
    #[test]
    fn direction_classification_and_opposites() {
        for (direction, horizontal, vertical, value) in [
            (Direction::None, false, false, 0),
            (Direction::Right, true, false, 1),
            (Direction::Left, true, false, -1),
            (Direction::Up, false, true, 2),
            (Direction::Down, false, true, -2),
        ] {
            assert_eq!(direction.is_horizontal(), horizontal);
            assert_eq!(direction.is_vertical(), vertical);
    ...
    #[test]
    fn metrics_constructors_default() {
        let axis = AfLatinAxisMetrics::new();
        assert_eq!(axis.width_count, 0);
        assert_eq!(axis.widths.len(), AF_LATIN_MAX_WIDTHS);
        assert!(axis.blues.is_empty());

        let metrics = AfLatinMetrics::new(1000, 3);
        assert_eq!(metrics.units_per_em, 1000);
        assert_eq!(metrics.non_base_glyphs.len(), 3);
        assert_eq!(metrics.digit_glyphs.len(), 3);
        assert!(metrics.digits_have_same_width);
        assert!(!metrics.fixed_width);
        assert!(!metrics.skip_xh_adjust);
        assert!(!metrics.no_advance_hinting);
    }

    #[test]
    fn glyph_hints_save_to_outline() {
        let mut hints = GlyphHints::new(1 << 16, 1 << 16, 0, 0);
        hints.points.push(AFPoint {
            x: 10,
            y: 20,
            ..AFPoint::default()
        });
        hints.contours.push(0);
        assert_eq!(hints.num_points(), 1);
        assert_eq!(hints.num_contours(), 1);
        let mut outline = Outline::default();
        outline.points.push(crate::outline::OutlinePoint {
            x: 0,
            y: 0,
            ..Default::default()
        });
    ...
    #[test]
    fn axis_hints_defaults() {
        let hints = AxisHints::new();
        assert!(hints.segments.is_empty());
        assert!(hints.edges.is_empty());
        assert_eq!(hints.major_dir, Direction::None);
    }
}
