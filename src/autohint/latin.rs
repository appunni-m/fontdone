...
pub fn metrics_scale_dim(
    ...
    metrics_scale_dim_with_increase_x_height(metrics, x_scale, y_scale, x_delta, y_delta, 0, 0)
}

/// Scale metrics with the face-scoped `increase-x-height` property.
///
/// Pinned FreeType changes the x-height rounding threshold from 40 to 52 only
/// for `6 <= x_ppem <= limit`; zero disables the property.
pub fn metrics_scale_dim_with_increase_x_height(
    metrics: &mut AfLatinMetrics,
    x_scale: i32,
    y_scale: i32,
    x_delta: i32,
    ...
    let mut v_scale = y_scale;
    {
        ...
        if let Some(ai) = adj_idx {
            ...
            let threshold: i32 = if increase_x_height != 0
                && x_ppem >= 6
                && u32::try_from(x_ppem).is_ok_and(|ppem| ppem <= increase_x_height)
            {
                52
            } else {
                40
