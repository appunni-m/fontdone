...
pub struct FaceGlobals {
    ...
    pub is_italic: bool,
    /// Last face-owned public glyph-style array synchronized by the FFI load
    /// boundary.  The array is caller-mutable, so equality is checked before
    /// every glyph load.
    public_glyph_styles: Rc<RefCell<Option<Vec<u16>>>>,
    /// Face-scoped `autofitter:increase-x-height` limit.
    increase_x_height: Rc<Cell<u32>>,
}
...
impl FaceGlobals {
    ...
    pub fn new(font_data: std::sync::Arc<FontData>, is_italic: bool) -> Self {
        ...
        FaceGlobals {
            ...
            font_data,
            is_italic,
            public_glyph_styles: Rc::new(RefCell::new(None)),
            increase_x_height: Rc::new(Cell::new(0)),
        }
    }

    /// Apply the face-owned public autofitter state before a glyph load.
    ///
    /// Pinned FreeType stores public style enum values plus `AF_*` flags in
    /// `glyph_styles`; Rust uses compact generated-table indexes internally.
    /// Convert at this boundary without clearing initialized style metrics:
    /// pinned C exposes the mutable map but retains each `AF_StyleMetrics`
    /// object once created.  Likewise, changing `increase-x-height` updates
    /// readback immediately but does not rescale metrics already cached for the
    /// current face and size.
    pub fn set_property_state(&self, glyph_styles: &[u16], increase_x_height: u32) {
        let map_changed = self
            .public_glyph_styles
            .borrow()
            .as_deref()
            .is_none_or(|current| current != glyph_styles);
        if map_changed {
            *self.public_glyph_styles.borrow_mut() = Some(glyph_styles.to_vec());
            *self.coverage.borrow_mut() = Some(coverage_from_public_glyph_styles(
                glyph_styles,
                self.glyph_count,
            ));
        }
        if self.increase_x_height.get() != increase_x_height {
            self.increase_x_height.set(increase_x_height);
        }
    }
...
fn coverage_from_public_glyph_styles(public_styles: &[u16], glyph_count: u16) -> FaceCoverage {
    let glyph_count = usize::from(glyph_count);
    let mut glyph_styles = vec![STYLE_FALLBACK; glyph_count];
    let mut non_base_glyphs = vec![false; glyph_count];
    let mut digit_glyphs = vec![false; glyph_count];
    for (glyph_index, value) in public_styles.iter().copied().take(glyph_count).enumerate() {
        let public_style = value & AF_STYLE_MASK;
        glyph_styles[glyph_index] = PINNED_FT_PUBLIC_STYLE_VALUES
            .iter()
            .position(|candidate| *candidate == public_style)
            .unwrap_or(STYLE_FALLBACK);
        non_base_glyphs[glyph_index] = value & AF_NONBASE != 0;
        digit_glyphs[glyph_index] = value & AF_DIGIT != 0;
    }
    FaceCoverage {
        glyph_styles,
        non_base_glyphs,
...
mod tests {
    ...
    #[test]
    fn writing_system_classification() {
        for tag in ["hani", "limb", "orya", "sylo", "tibt"] {
            assert!(uses_cjk_writing_system(tag), "{tag} should be CJK");
        }
        assert!(!uses_cjk_writing_system("latn"));
        assert!(!uses_cjk_writing_system("deva"));

        for tag in ["limb", "orya", "sylo", "tibt"] {
            assert!(uses_indic_writing_system(tag), "{tag} should be Indic");
        }
        assert!(!uses_indic_writing_system("hani"));
        assert!(!uses_indic_writing_system("latn"));
    }

    #[test]
    fn top_to_bottom_scripts() {
        for tag in ["beng", "deva", "goth", "guru", "mong"] {
            assert!(top_to_bottom_hinting(tag), "{tag} should be top-to-bottom");
        }
        for tag in ["latn", "hani", "arab", "cyrl"] {
            assert!(!top_to_bottom_hinting(tag), "{tag} should be bottom-to-top");
        }
    }

    #[test]
    fn standard_chars_are_available() {
        assert_eq!(
            standard_chars_for_script("latn"),
            &['\u{006F}', '\u{004F}', '\u{0030}']
        );
        assert_eq!(standard_chars_for_script("none"), &[]);
        assert_eq!(
            standard_chars_for_script("deva"),
            &['\u{0920}', '\u{0935}', '\u{091F}']
        );
        assert_eq!(
            crate::autohint::globals_data::standard_char_for_script("latn"),
            'o'
        );
    }

    #[test]
    fn style_table_has_latin_and_fallback() {
        assert!(!STYLE_TABLE.is_empty());
        assert!(STYLE_TABLE.iter().any(|style| style.script_tag == "latn"));
        assert_ne!(STYLE_FALLBACK, STYLE_UNASSIGNED);
    }
}
