...
mod tests {
    ...
    fn gasp(version: u16, ranges: &[(u16, u16)]) -> Vec<u8> {
        let mut data = Vec::new();
        data.extend_from_slice(&version.to_be_bytes());
        data.extend_from_slice(&(ranges.len() as u16).to_be_bytes());
        for (max_ppem, flags) in ranges {
            data.extend_from_slice(&max_ppem.to_be_bytes());
    ...
    #[test]
    fn parses_ranges_and_masks_version_zero_flags() {
        let table = match parse_gasp(&gasp(0, &[(10, 0x0002), (24, 0x000F)])) {
            Ok(table) => table,
            Err(error) => panic!("gasp rejected: {error}"),
        };
        assert_eq!(table.get(5), 0x0002);
        assert_eq!(table.get(10), 0x0002);
        // Version 0 masks flags to the legacy two bits.
        assert_eq!(table.get(24), 0x0003);
        assert_eq!(table.get(100), FT_GASP_NO_TABLE);
    }

    #[test]
    fn version_one_keeps_full_flags() {
        let table = match parse_gasp(&gasp(1, &[(16, 0x000F)])) {
            Ok(table) => table,
            Err(error) => panic!("gasp v1 rejected: {error}"),
        };
        assert_eq!(table.get(16), 0x000F);
    }

    #[test]
    fn rejects_short_unsupported_and_truncated_tables() {
        assert!(parse_gasp(&[0; 3]).is_err());
        let error = match parse_gasp(&gasp(2, &[])) {
            Err(error) => error,
            Ok(_) => panic!("gasp v2 should be rejected"),
        };
        assert!(error.to_string().contains("unsupported gasp table version"));
        let mut data = gasp(1, &[(10, 1)]);
        data[2..4].copy_from_slice(&3u16.to_be_bytes()); // claims 3 ranges
