...
mod tests {
    ...
    fn metric(advance: u16, tsb: i16) -> Vec<u8> {
        let mut bytes = advance.to_be_bytes().to_vec();
        bytes.extend_from_slice(&tsb.to_be_bytes());
        bytes
    }

    #[test]
    fn parses_explicit_and_fallback_metrics() -> Result<(), FontError> {
        let mut data = metric(700, -5);
        data.extend_from_slice(&metric(800, 3));
        // glyphs 2..5 fall back to the last advance with their own tsb.
        for tsb in [1i16, -2, 4] {
            data.extend_from_slice(&tsb.to_be_bytes());
        }
        let table = parse_vmtx(&data, 2, 5)?;
        assert_eq!(table.v_metrics.len(), 2);
        assert_eq!(
            table.v_metrics[0],
            LongVerMetric {
                advance_height: 700,
        ...
        assert_eq!(table.top_side_bearings, vec![1, -2, 4]);

        let first = table.get(0);
        assert_eq!(first.advance_height, 700);
        assert_eq!(first.tsb, -5);
        let past = table.get(3);
        assert_eq!(past.advance_height, 800);
        assert_eq!(past.tsb, -2);
        let out_of_range = table.get(9);
        assert_eq!(out_of_range.advance_height, 800);
        assert_eq!(out_of_range.tsb, 0);
        Ok(())
    }

    #[test]
    fn empty_fallback_metrics_default_to_zero() {
        let table = VmtxTable::default();
        assert_eq!(table.get(0).advance_height, 0);
        assert_eq!(table.get(0).tsb, 0);
    }

    #[test]
    fn rejects_out_of_range_metric_count() {
        let error = match parse_vmtx(&[0u8; 8], 3, 2) {
            Err(error) => error,
            Ok(_) => panic!("vm_count > num_glyphs should be rejected"),
        };
        assert!(error.to_string().contains("num_vmetrics out of range"));
        let error = match parse_vmtx(&[], 0, 5) {
            Err(error) => error,
            Ok(_) => panic!("zero vm_count should be rejected"),
        };
        assert!(error.to_string().contains("num_vmetrics out of range"));
    }
    ...
    fn rejects_truncated_table() {
        let error = match parse_vmtx(&[0u8; 4], 2, 2) {
            Err(error) => error,
            Ok(_) => panic!("short vmtx should be rejected"),
        };
        assert!(error.to_string().contains("vmtx table too short"));
    }
}
