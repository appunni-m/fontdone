...
impl CmapTable {
    pub(crate) fn from_sparse_mapping(
        platform_id: u16,
        encoding_id: u16,
        mappings: &[(u32, u16)],
    ) -> Self {
        let mut mappings = mappings
            ...
            .collect::<Vec<_>>();
        mappings.sort_unstable_by_key(|(codepoint, _)| *codepoint);
        mappings.dedup_by_key(|(codepoint, _)| *codepoint);

        let subtable = Format12Subtable {
            platform_id,
            encoding_id,
            language_id: 0,
            start_codes: mappings.iter().map(|(codepoint, _)| *codepoint).collect(),
            end_codes: mappings.iter().map(|(codepoint, _)| *codepoint).collect(),
            start_glyph_ids: mappings
                .iter()
                .map(|(_, glyph)| u32::from(*glyph))
                .collect(),
        };
        Self {
            charmaps: vec![CharmapRecord {
                platform_id,
                encoding_id,
                // Non-SFNT drivers report -1 through `FT_Get_CMap_Format`.
                // This internal value only selects the sparse lookup backend;
                // the C facade records the non-SFNT public format separately.
                format: 0,
                language_id: 0,
                kind: CharmapKind::Format12(0),
            }],
            format12: vec![subtable],
            ..Self::default()
        }
    }
    ...
    pub fn first_char(&self, charmap_index: usize) -> Option<(u32, u16)> {
        // FreeType `FT_Get_First_Char` first performs the ordinary index
        // lookup for U+0000 and calls `FT_Get_Next_Char` only when that glyph
        // is zero.  Calling `next_char(..., 0)` unconditionally loses the
        // valid `(charcode=0, glyph>0)` result.
        if let Some(glyph) = self
            .char_index_in_charmap(charmap_index, 0)
...
mod tests {
    ...
    /// Build a cmap header with one encoding record pointing at `subtable`.
    fn cmap_with(platform: u16, encoding: u16, subtable: &[u8]) -> Vec<u8> {
    ...
    /// Build a cmap header with a format-6 base record followed by a
    /// format-14 record (used for variation-selector resolution).
    fn cmap_with_two(first: &[u8], second: &[u8]) -> Vec<u8> {
        let mut data = Vec::new();
        data.extend_from_slice(&0u16.to_be_bytes()); // version
        data.extend_from_slice(&2u16.to_be_bytes()); // two records
        data.extend_from_slice(&0u16.to_be_bytes()); // platform 0
        data.extend_from_slice(&6u16.to_be_bytes());
        data.extend_from_slice(&20u32.to_be_bytes()); // first subtable offset
        data.extend_from_slice(&0u16.to_be_bytes()); // platform 0
        data.extend_from_slice(&5u16.to_be_bytes());
        data.extend_from_slice(&(20 + first.len() as u32).to_be_bytes());
    ...
    #[test]
    fn parses_format6_trimmed_mapping() -> Result<(), FontError> {
        let mut sub = Vec::new();
        sub.extend_from_slice(&6u16.to_be_bytes());
        sub.extend_from_slice(&14u16.to_be_bytes()); // length
        sub.extend_from_slice(&0u16.to_be_bytes()); // language
        sub.extend_from_slice(&65u16.to_be_bytes()); // first code
        sub.extend_from_slice(&2u16.to_be_bytes()); // entry count
        sub.extend_from_slice(&1u16.to_be_bytes());
        sub.extend_from_slice(&2u16.to_be_bytes());
        let table = parse_cmap(&cmap_with(0, 6, &sub))?;
        assert_eq!(table.char_index_in_charmap(0, 65), Some(1));
        assert_eq!(table.char_index_in_charmap(0, 66), Some(2));
        assert_eq!(table.char_index_in_charmap(0, 67), None);
        assert_eq!(table.char_index_in_charmap(0, 0x1_0000), None);
        assert_eq!(table.first_char(0), Some((65, 1)));
        assert_eq!(table.next_char(0, 65), Some((66, 2)));
        assert_eq!(table.next_char(0, 66), None);
        Ok(())
    }

    #[test]
    fn parses_format12_grouped_mapping() -> Result<(), FontError> {
        let mut sub = Vec::new();
        ...
        sub.extend_from_slice(&1u32.to_be_bytes()); // one group
        sub.extend_from_slice(&0x1000u32.to_be_bytes()); // start
        sub.extend_from_slice(&0x1002u32.to_be_bytes()); // end
        sub.extend_from_slice(&5u32.to_be_bytes()); // start glyph
        let table = parse_cmap(&cmap_with(3, 10, &sub))?;
        assert_eq!(table.char_index(0x1000), Some(5));
        assert_eq!(table.char_index(0x1002), Some(7));
        assert_eq!(table.char_index(0x1003), None);
        assert_eq!(table.char_index(0x0FFF), None);
        assert_eq!(table.first_char(0), Some((0x1000, 5)));
        assert_eq!(table.next_char(0, 0x1001), Some((0x1002, 7)));
        Ok(())
    }

    #[test]
    fn parses_format13_many_to_one_mapping() -> Result<(), FontError> {
        let mut sub = Vec::new();
        sub.extend_from_slice(&13u16.to_be_bytes());
        ...
        sub.extend_from_slice(&0x2000u32.to_be_bytes());
        sub.extend_from_slice(&0x2001u32.to_be_bytes());
        sub.extend_from_slice(&9u32.to_be_bytes());
        let table = parse_cmap(&cmap_with(3, 13, &sub))?;
        assert_eq!(table.char_index_in_charmap(0, 0x2000), Some(9));
        assert_eq!(table.char_index_in_charmap(0, 0x2001), Some(9));
        assert_eq!(table.char_index_in_charmap(0, 0x2002), None);
        Ok(())
    }

    #[test]
    fn parses_format14_variation_selectors() -> Result<(), FontError> {
        // Format 14 with one selector record: default ranges point to a
        // default UVS table; a non-default UVS table maps 0x4B -> glyph 0x10.
        let mut sub = Vec::new();
        sub.extend_from_slice(&14u16.to_be_bytes());
        sub.extend_from_slice(&35u32.to_be_bytes()); // length (patched below)
        sub.extend_from_slice(&1u32.to_be_bytes()); // one record
        sub.extend_from_slice(&[0x00, 0xFE, 0x0F]); // selector = 0xFE0F (24-bit)
        sub.extend_from_slice(&21u32.to_be_bytes()); // default UVS offset
        sub.extend_from_slice(&0u32.to_be_bytes()); // non-default offset (patched)
        sub.extend_from_slice(&1u32.to_be_bytes()); // one range
        // Default range record: 24-bit start + 1-byte additional count.
        sub.extend_from_slice(&[0x00, 0x00, 0x41]); // start 0x41
        sub.push(2); // covers 0x41, 0x42, 0x43
        // Non-default UVS table follows: one mapping 0x4B -> glyph 0x10.
        let non_default_offset = sub.len() as u32;
        sub.extend_from_slice(&1u32.to_be_bytes()); // count
        sub.extend_from_slice(&[0x00, 0x00, 0x4B]); // codepoint 0x4B
        sub.extend_from_slice(&0x10u16.to_be_bytes()); // glyph
        // Patch the record's non-default offset (bytes 17-20) and length.
        sub[17..21].copy_from_slice(&non_default_offset.to_be_bytes());
        let sub_len = sub.len() as u32;
        sub[2..6].copy_from_slice(&sub_len.to_be_bytes());

        // Format 6 base subtable maps 0x41 -> glyph 0x41.
        let mut base = Vec::new();
        base.extend_from_slice(&6u16.to_be_bytes());
        base.extend_from_slice(&12u16.to_be_bytes()); // length
        base.extend_from_slice(&0u16.to_be_bytes()); // language
        base.extend_from_slice(&0x41u16.to_be_bytes()); // first code
        base.extend_from_slice(&1u16.to_be_bytes()); // one entry
        base.extend_from_slice(&0x41u16.to_be_bytes());
        let table = parse_cmap(&cmap_with_two(&base, &sub))?;
        assert_eq!(table.char_variant_index(0, 0x41, 0xFE0F), 0x41);
        assert_eq!(table.char_variant_index(0, 0x4B, 0xFE0F), 0x10);
        assert_eq!(table.char_variant_is_default(0x42, 0xFE0F), 1);
        assert_eq!(table.char_variant_is_default(0x4B, 0xFE0F), 0);
        assert_eq!(table.char_variant_is_default(0x44, 0xFE0F), -1);
        assert_eq!(table.variant_selectors(), Some(vec![0xFE0F]));
        assert_eq!(table.variants_of_char(0x41), Some(vec![0xFE0F]));
        assert!(table.chars_of_variant(0xFE0F).is_some());
        assert!(table.chars_of_variant(0xE0100).is_none());
        Ok(())
    }

    #[test]
    fn rejects_truncated_cmap() {
        assert!(parse_cmap(&[0; 3]).is_err());
        let mut data = vec![0u8; 4];
        data[2..4].copy_from_slice(&2u16.to_be_bytes());
        assert!(parse_cmap(&data).is_err());
        // Encoding record pointing past the end of the table is skipped.
        let data = cmap_with(0, 4, &[0, 0]);
        assert!(parse_cmap(&data).is_ok());
    }
}
