...
//!
//! This module intentionally mirrors common FreeType concepts without exposing
//! raw pointers or runtime FFI. A [`Face`] owns the parsed pure-Rust font, and [`GlyphSlot`] is an
...
use crate::tt::{hinter::NativeHintMode, loca};
...
pub enum GlyphFormat {
    ...
    Bitmap,
    /// An OpenType SVG document is present.
    Svg,
}
...
impl LoadFlags {
    ...
    /// Permit OpenType SVG glyph documents, equivalent to `FT_LOAD_COLOR`.
    pub const COLOR: Self = Self(1 << 17);
    /// Disable OpenType SVG glyph documents, equivalent to `FT_LOAD_NO_SVG`.
    pub const NO_SVG: Self = Self(1 << 18);
    /// Return an error unless an SVG document is loaded.
    pub const SVG_ONLY: Self = Self(1 << 19);
...
impl Library {
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] when `data` is unsupported or malformed, the
    /// face index is invalid, or required tables cannot be parsed.
...
impl Face {
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] when the bytes or face index cannot produce a
    /// supported face.
    ...
    pub(crate) fn try_set_pixel_sizes(
        ...
        self.font.try_set_pixel_sizes(pixel_width, pixel_height)?;
        self.render_fonts.clear();
        Ok(())
    }
    ...
    /// # Errors
    ///
    /// Returns [`SizeRequestError`] when the requested scale or ppem is
    /// invalid for the active face.
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] when `index` is outside the face's charmap list.
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] when the mapped glyph cannot be loaded, hinted,
    /// or rendered under `flags`.
    ...
    /// # Errors
    ///
    /// Returns [`FontError`] when the glyph index or requested load/render
    ...
    pub(crate) fn incremental_glyph_data(&self, glyph_index: u16) -> Option<&[u8]> {
        let data = &self.font.data;
        let location = loca::get_glyph_location(
            &data.loca_data,
            glyph_index,
        ...
        data.glyf_data.get(start..end)
    }

    /// Load a glyph index with an optional transform, equivalent to
    /// `FT_Load_Glyph` + `FT_Set_Transform`.
    ///
    /// Matrix coefficients are signed 16.16, while the final two tuple values
    /// are 26.6 translation coordinates.
    ///
    /// # Errors
    ///
    /// Returns [`FontError`] when loading, hinting, transforming, or optional
    /// rendering fails.
    ...
    fn load_glyph_from_font(
        ...
        if svg_allowed {
            match font.load_svg_glyph(glyph_index) {
                Ok((metrics, document)) => {
                    let mut slot =
                        GlyphSlot::from_svg(glyph_index, metrics, document, vertical_layout);
                    if let Some((xx, xy, yx, yy, dx, dy)) = transform {
                        slot.apply_transform(xx, xy, yx, yy, dx, dy);
                    }
                    return Ok(slot);
    ...
    /// Load one glyph index and render it with the requested mode.
    ///
    /// The returned slot owns its bitmap bytes. Metrics are in 26.6 pixels
    /// unless `load_flags` requests unscaled output.
    ///
    /// # Errors
    ///
    /// Returns [`FontError`] when the glyph cannot be loaded, hinted, or
    /// rasterized for the active face and size.
...
impl GlyphSlot {
    ...
    pub(crate) fn render_with_ppem(
        ...
        if self.format == GlyphFormat::Svg {
            return Err(FontError::CannotRenderGlyph(
                "SVG renderer is not installed".to_string(),
            ));
        }
    ...
    pub fn apply_transform(&mut self, xx: i32, xy: i32, yx: i32, yy: i32, dx: i32, dy: i32) {
        ...
        if let Some(document) = self.svg.as_mut() {
            let (old_xx, old_xy, old_yx, old_yy) = document.transform;
            document.transform = (
                crate::fixed::ft_mul_fix(xx, old_xx)
                    .wrapping_add(crate::fixed::ft_mul_fix(xy, old_yx)),
                crate::fixed::ft_mul_fix(xx, old_xy)
                    .wrapping_add(crate::fixed::ft_mul_fix(xy, old_yy)),
                crate::fixed::ft_mul_fix(yx, old_xx)
                    .wrapping_add(crate::fixed::ft_mul_fix(yy, old_yx)),
                crate::fixed::ft_mul_fix(yx, old_xy)
                    .wrapping_add(crate::fixed::ft_mul_fix(yy, old_yy)),
            );
            let (old_dx, old_dy) = document.delta;
            document.delta = (
                crate::fixed::ft_mul_fix(xx, old_dx)
                    .wrapping_add(crate::fixed::ft_mul_fix(xy, old_dy))
                    .wrapping_add(dx),
                crate::fixed::ft_mul_fix(yx, old_dx)
                    .wrapping_add(crate::fixed::ft_mul_fix(yy, old_dy))
                    .wrapping_add(dy),
            );
        }
