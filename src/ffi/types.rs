...
#![expect(
    missing_docs,
    reason = "mechanical FreeType records are documented by the public ffi facade and generated compatibility contracts"
)]
...
pub type FT_Outline_LineToFunc =
    Option<unsafe extern "C" fn(to: *const FT_Vector, user: FT_Pointer) -> c_int>;
pub type FT_Outline_LineTo_Func = FT_Outline_LineToFunc;
...
>;
pub type FT_Outline_ConicTo_Func = FT_Outline_ConicToFunc;
...
>;
pub type FT_Outline_CubicTo_Func = FT_Outline_CubicToFunc;
...
pub type FT_Raster_BitTest_Func =
    Option<unsafe extern "C" fn(y: c_int, x: c_int, user: FT_Pointer) -> c_int>;
pub type FT_Raster_BitSet_Func = Option<unsafe extern "C" fn(y: c_int, x: c_int, user: FT_Pointer)>;
...
pub type FT_Renderer_GetCBoxFunc =
    Option<unsafe extern "C" fn(renderer: FT_Renderer, slot: FT_Pointer, cbox: *mut FT_BBox)>;
...
/// Owned OpenType SVG document attached to a loaded glyph slot.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FT_SvgDocumentOwned {
