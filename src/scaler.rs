...
pub(crate) fn prepare_native_bytecode_context(
    ...
    let context_scale = crate::tt::hinter::HintScale {
        ...
        max_stack_elements: data.maxp.max_stack_elements as usize,
        num_glyphs: data.maxp.num_glyphs as usize,
...
fn scale_glyph_impl_with_scale(
    ...
    if !data.has_cff_outlines() && !allow_bytecode && latin_metrics.is_none() {
    ...
    let no_hinting_origin_shift_x = if !data.has_cff_outlines()
        && !use_autohint
        && (!allow_bytecode || !can_execute_native_bytecode)
    {
    ...
    let hdmx_slot_advance_width =
        if use_hdmx && !legacy_hinter_phantoms && allow_bytecode && !data.has_active_variation() {
            data.hdmx
                .as_ref()
...
fn vertical_top_and_advance_font_units(
    ...
    if let Some(vmtx) = &data.vmtx {
        ...
        let height_fu = if !data.has_cff_outlines() {
